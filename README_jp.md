...
### 3. Tesseract OCR のインストール（オプショナル）
...
- manga-ocrを使用するには`uv sync --extra manga-ocr`が必要です
- sarashina2.2-ocrを使用するには`uv sync --extra sarashina`が必要です（GPU推奨）
