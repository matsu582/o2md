...
class ExcelToMarkdownConverter(_TablesMixin, _GraphicsMixin):
    ...
    class _LoggingList(list):
        ...
        def append(self, item):
            logger.debug(f"[MD_APPEND] {repr(item)}")
            try:
    ...
    def __init__(self, excel_file_path: str, output_dir=None, shape_metadata=False, output_format='png'):
        ...
        logger.info(f"Excelワークブック読み込み完了: {excel_file_path}")
    ...
    def _mark_image_emitted(self, img_name: str):
        ...
        if self._is_canonical_emit():
            ...
            logger.debug(f"[TRACE] Skipping _emitted_images.add({img_name}) in non-canonical pass")
    ...
    def _mark_sheet_map(self, sheet_title: str, src_row: int, md_index: int):
        ...
        if self._is_canonical_emit():
            ...
            logger.debug(f"[TRACE] Skipping authoritative sheet_map[{sheet_title}][{src_row}] assignment in non-canonical pass")
    ...
    def _mark_emitted_row(self, sheet_title: str, row: int):
        ...
        if self._is_canonical_emit():
            ...
            logger.debug(f"[TRACE] Skipping emitted_rows.add({sheet_title},{row}) in non-canonical pass")
    ...
    def _mark_emitted_text(self, sheet_title: str, norm_text: str):
        ...
        if self._is_canonical_emit():
            ...
            logger.debug(f"[TRACE] Skipping emitted_texts.add({sheet_title},...) in non-canonical pass")
    ...
    def _add_separator(self):
        ...
        for t in reversed(tail):
            if t.strip() == '':
                continue
            if t.strip() == '---':
                logger.debug("[DEBUG][_add_separator] skipping duplicate separator '---'")
                return False
            break
        ...
        try:
            ...
            if self._is_canonical_emit():
                ...
                if src_row is not None:
                    ...
                    logger.debug(f"[DEBUG][_text_emit] sheet={sheet.title} src_row={src_row} md_index={md_index} text_norm='{norm}'")
            ...
            logger.error(f"_emit_free_text failed: {e}")
            return False
    ...
    def _insert_markdown_image(self, insert_at: Optional[int], md_line: str, img_name: str, sheet=None):
        ...
        try:
            ...
            logger.debug(f"[DEBUG][_insert_markdown_image_called] insert_at={insert_at} img_name={img_name} caller={caller_info}")
            # 正規の出力パス中でなく、即座の画像
            ...
            if not getattr(self, '_in_canonical_emit', False) and not getattr(self, '_allow_immediate_image_inserts', False):
                ...
                if not already:
                    ...
                    logger.debug(f"[DEBUG][_insert_markdown_image_deferred] img_name={img_name} sheet={key}")
            ...
            if insert_at is None:
                ...
                if sheet is not None:
                    try:
                        ...
                        logger.warning(f"Failed to add shape metadata: {e}")
                    ...
                    for fname in os.listdir(xl_worksheets):
                        if fname.endswith('.xml') and fname.startswith('sheet'):
                            ...
                            try:
                                ...
                                logger.debug(f"[DEBUG] {fname} のpageSetupを縦横1ページに設定")
                            except Exception as e:
    ...
    def _convert_sheet(self, sheet):
        ...
        try:
            ...
            logger.warning(f"シートヘッダー処理でエラー: {e}")
            # エラー時はdata_rangeを再取得
    ...
    def _process_sheet_charts(self, sheet):
        ...
        try:
            ...
            logger.info(f"シート '{sheet.title}' から {len(ws_charts)} 個のチャートを画像化")
            ...
            logger.warning(f"チャート処理中にエラー: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _render_chart_as_image(self, sheet, chart, chart_index: int):
        ...
        try:
            ...
            logger.warning(f"チャート画像レンダリングエラー: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _convert_chart_excel_to_pdf(self, xlsx_path: str):
        ...
        try:
            ...
            if not is_libreoffice_available():
                logger.debug("[DEBUG] LibreOfficeが利用できないため、チャートのPDF変換をスキップします")
                return None
            ...
            if result.returncode != 0:
                logger.debug(f"[DEBUG] LibreOffice変換エラー: {result.stderr}")
            ...
            if os.path.exists(pdf_path):
                ...
                shutil.rmtree(temp_dir)
                logger.info(f"Excel→PDF変換完了: {final_pdf}")
                return final_pdf
            ...
            logger.warning(f"Excel→PDF変換エラー: {e}")
            return None
    ...
    def _convert_chart_pdf_to_svg(self, pdf_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"チャートSVG変換完了: {output_path}")
            return True
            
        except Exception as e:
            logger.warning(f"チャートSVG変換エラー: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _convert_chart_pdf_to_png(self, pdf_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"チャートPNG変換完了: {output_path}")
            return True
            
        except Exception as e:
            logger.warning(f"チャートPNG変換エラー: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _trim_chart_margins(self, img):
        ...
        try:
            ...
            logger.debug(f"[DEBUG] 余白トリムエラー: {e}")
            return img
    ...
    def _reorder_sheet_output_by_row_order(self, sheet):
        ...
        try:
            ...
            logger.debug(f"[DEBUG][_reorder_entry] sheet={sheet.title}")
            # 実行毎のディスク上マーカーを削除; デバッグトレースのみを保持。
            logger.debug(f"[DEBUG][_reorder_entry_marker] sheet={sheet.title}")
            ...
            try:
                logger.debug(f"[DEBUG][_canonical_enter] sheet={sheet.title} emitted_rows={sorted(list(emitted_rows))[:50]} deferred_texts_count={len(deferred_texts)}")
            except (ValueError, TypeError):
                logger.debug(f"[DEBUG][_canonical_enter] sheet={getattr(sheet, 'title', None)} emitted_rows=<error> deferred_texts_count=<error>")
            ...
            try:
                for rr, fns in list(img_map.items()):
                    ...
                    if has_group:
                        ...
                        if suppressed:
                            logger.debug(f"[DEBUG][_img_suppress] sheet={sheet.title} row={rr} suppressed={suppressed} kept={kept}")
                        img_map[rr] = kept
            ...
            try:
                ...
                try:
                    for e in events_emit:
                        ...
                        logger.debug(f"[DEBUG][_events_emit_sorted] sheet={sheet.title} {log_line}")
                except (ValueError, TypeError) as e:
                    logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                ...
                logger.debug(f"[DEBUG][_events_sorted] sheet={sheet.title} events_count_emit={len(events_emit)}")
                ...
                try:
                    logger.debug(f"[DEBUG][_sorted_events_block] sheet={sheet.title} events_count={len(events_emit)}")
                    ...
                    if kind == 'text':
                        ...
                        if not emitted_ok:
                            ...
                            try:
                                ...
                                logger.debug(f"[DEBUG][_emit_fallback] row={row} text={txt} >>")
                        ...
                        try:
                            ...
                            logger.debug(f"[DEBUG][_emit_table] row={row} table_rows={len(table_data) if isinstance(table_data, list) else 0} src_rows={src_rows} meta={meta} >>")
                            ...
                            try:
                                ...
                                if title:
                                    ...
                                    if normalized_title in self._sheet_emitted_table_titles[sheet.title]:
                                        logger.debug(f"[DEBUG] タイトル '{title}' は既に出力済みのため抑制")
                                                ...
                                                if isinstance(header_row, (list, tuple)):
                                                    ...
                                                    if normalized_title and normalized_header:
                                                        if normalized_title in normalized_header or normalized_header.startswith(normalized_title):
                                                            should_emit_title = False
                                                            logger.debug(f"[DEBUG] タイトル '{title}' はヘッダー行と重複しているため出力を抑制")
                                        except Exception as e:
                                            logger.debug(f"[DEBUG] タイトル冗長性チェックエラー（無視）: {e}")
                                            should_emit_title = True
                        ...
                        img_fn = payload
                        logger.debug(f"[DEBUG][_emit_image] row={row} image={img_fn} >> ")
                        ref = f"images/{img_fn}"
                        # 重複画像を決定論的にゲート
                        ...
                        try:
                            ...
                            if shapes_metadata:
                                logger.debug(f"[DEBUG] 図形メタデータ抽出成功: {img_fn} -> {len(shapes_metadata)} shapes")
                                text_metadata = self._format_shape_metadata_as_text(shapes_metadata)
                        ...
                        try:
                            ...
                            logger.debug(f"WARNING self._mark_sheet_map({sheet.title}, {row}, {md_idx})")
                        try:
                            # 重複を防ぐためemitted_imagesを常にマーク。これは安全です。
                            # emitted_imagesはファイル名のみを追跡し、プルーニングに影響しないため。
                            self._mark_image_emitted(img_fn)
                        except (ValueError, TypeError):
                            logger.debug(f"WARNING self._mark_image_emitted({img_fn})")
            except (ValueError, TypeError):
            ...
            for img in remaining_imgs:
                ...
                try:
                    ...
                    logger.debug(f"WARNING: Exception self._mark_sheet_map({sheet.title}, {1}, {len(self.markdown_lines) - 2})")
                try:
                    self._mark_image_emitted(img)
                except (ValueError, TypeError):
                    logger.debug(f"WARNING: Exception self._mark_image_emitted({img})")
            ...
            logger.debug(f"[DEBUG][_reorder_exception] sheet={sheet.title} exc={_exc!r}")
            import traceback
            traceback.print_exc()
    ...
    def _prune_emitted_rows(self, sheet_title: str, table_data: List[List[str]], source_rows: Optional[List[int]]):
        ...
        try:
            ...
            logger.debug(f"[TRACE][_prune_emitted_rows_entry] sheet={sheet_title} emitted_count_total={len(emitted)} emitted_count_auth={len(authoritative_emitted)} emitted_sample={sample_emitted} source_rows_count={len(source_rows) if source_rows else 0}")
        except (ValueError, TypeError):
            logger.debug(f"[TRACE][_prune_emitted_rows_entry] sheet={sheet_title} unable to snapshot emitted set")
        ...
        for row, src in zip(table_data, source_rows):
            try:
                ...
                if src not in authoritative_emitted:
                    ...
                    logger.debug(f"[TRACE][_prune_emitted_rows_removed] sheet={sheet_title} removed_src_row={src}")
            except (ValueError, TypeError):
                pruned_table.append(row)
                pruned_src.append(src)

        logger.debug(f"[TRACE][_prune_emitted_rows_exit] sheet={sheet_title} in={len(source_rows)} out={len(pruned_src)}")
    ...
    def _convert_sheet_data(self, sheet, data_range: Tuple[int, int, int, int]):
        ...
        logger.info(f"データ範囲: 行{min_row}〜{max_row}, 列{min_col}〜{max_col}")
        
        # 罫線で囲まれた矩形領域のみを表として抽出
        logger.info("罫線で囲まれた領域によるテーブル抽出を開始...")
        table_regions = self._detect_bordered_tables(sheet, min_row, max_row, min_col, max_col)
        logger.debug(f"[DEBUG][_convert_sheet_data] bordered_table_regions_count={len(table_regions)} sample={table_regions[:5]}")
        ...
        if table_regions:
            logger.debug(f"[DEBUG] bordered tables found; trying discrete region detection for non-occupied areas")
            try:
                ...
                if discrete_regions:
                    ...
                    if new_discrete_regions:
                        logger.debug(f"[DEBUG] adding {len(new_discrete_regions)} non-overlapping discrete regions")
                        table_regions = table_regions + new_discrete_regions
            except Exception as _e:
                logger.debug(f"[DEBUG] discrete region detection failed: {_e}")
        ...
        if not table_regions or not top_region_in_bordered:
            try:
                if not table_regions:
                    logger.debug("[DEBUG] no bordered tables found; trying heuristic _detect_table_regions first")
                    ...
                    if heur_tables:
                        logger.debug(f"[DEBUG] heuristic detection found {len(heur_tables)} table regions")
                        table_regions = heur_tables
                    else:
                        # 離散領域検出は一時的に無効化（テキストがテーブルとして検出される問題を回避）
                        logger.debug("[DEBUG] heuristic detection failed; discrete region detection is disabled")
                else:
                    logger.debug(f"[DEBUG] bordered tables found but no top region (rows 1-4); trying heuristic _detect_table_regions to find header rows")
                    heur_tables, heur_annotations = self._detect_table_regions(sheet, min_row, max_row, min_col, max_col)
                    try:
                        logger.debug(f"[TRACE][_detect_table_regions_result] sheet={sheet.title} heur_tables_count={len(heur_tables) if heur_tables else 0} heur_annotations_count={len(heur_annotations) if heur_annotations else 0}")
                        if heur_tables:
                            logger.debug(f"[TRACE][_detect_table_regions_result_sample] {heur_tables[:10]}")
                        if heur_annotations:
                            logger.debug(f"[TRACE][_detect_table_regions_annotations_sample] {heur_annotations[:10]}")
                    except (ValueError, TypeError) as e:
                        logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                    if heur_tables:
                        top_heur_tables = [r for r in heur_tables if r[0] == 1 and r[1] <= 4]
                        if top_heur_tables:
                            logger.debug(f"[DEBUG] adding {len(top_heur_tables)} top regions from heuristic detection to bordered tables")
                            table_regions = top_heur_tables + table_regions
            except Exception as _e:
                logger.debug(f"[DEBUG] table detection failed: {_e}")
        ...
        def region_overlaps_drawings(region, drawing_ranges, overlap_threshold=0.25):
            ...
            if is_very_large_table and len(drawing_ranges) >= 10:
                ...
                logger.debug(f"[DEBUG] Large table with many drawings detected - using stricter threshold {adjusted_threshold}")
            else:
                adjusted_threshold = overlap_threshold
            
            logger.debug(f"[DEBUG] table_region={region} overlap_cells={overlap_cells} table_cells={table_cells} frac={frac:.3f} threshold={adjusted_threshold:.3f}")
        ...
        for tr in table_regions:
            ...
            if region_overlaps_drawings(tr, drawing_cell_ranges):
                logger.info(f"テーブル領域が描画と重複しているため除外: {tr}")
                ...
                if (r1_b <= r1_a and r2_a <= r2_b and 
                    ...
                    if width_a == 1 and width_b >= 2:
                        ...
                        if overlap_ratio > 0.8:
                            logger.debug(f"[DEBUG] ネストされた1列テーブルを除外: {region_a} (含まれる先: {region_b}, 幅: {width_a} vs {width_b}, 重複率: {overlap_ratio:.2f})")
                            is_nested = True
                            break
        ...
        logger.debug(f"[DEBUG][_convert_sheet_data] kept_table_regions_count={len(table_regions)} kept_sample={table_regions[:5]}")
            ...
            for j, other_region in enumerate(table_regions):
                if i != j and regions_overlap(region, other_region):
                    ...
                    if r1_cells < r2_cells:
                        logger.debug(f"[DEBUG] 重複テーブルを除外: {region} (重複先: {other_region})")
                        is_duplicate = True
                        break
        ...
        logger.debug(f"[DEBUG][_convert_sheet_data] deduplicated_table_regions_count={len(table_regions)} deduplicated_sample={table_regions[:5]}")
        ...
        for region in table_regions:
            logger.debug(f"[DEBUG] emitting detected table region: {region}")
            try:
                ...
                logger.debug(f"[DEBUG] _convert_table_region failed for region={region}: {_e}")
        ...
        if 'excluded_table_regions' in locals() and excluded_table_regions:
            for excl in excluded_table_regions:
                try:
                    logger.info(f"描画重複のためプレーンテキストとして収集: {excl}")
        ...
        if processed_rows:
            try:
                ...
                logger.debug(f"[DEBUG] filtered merged_texts: removed {before_count - len(merged_texts)} rows that were already processed as tables")
            except (ValueError, TypeError) as e:
                logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                ...
                if (erow - srow + 1) >= 3:
                    ...
                    if cols_used:
                        ...
                        if self._is_colon_separated_list(sheet, srow, erow, smin, smax):
                            logger.debug(f"[DEBUG] implicit table is colon-separated list; skipping rows={srow}-{erow}")
                            continue
                        ...
                        if not skip_run:
                            ...
                            logger.debug(f"[DEBUG] implicit table detected rows={srow}-{erow} cols={smin}-{smax}")
        ...
        if processed_rows:
            try:
                ...
                merged_texts = [t for t in merged_texts if t[0] not in processed_rows]
                logger.debug(f"[DEBUG] post-implicit-filter: removed {before_count2 - len(merged_texts)} rows processed by implicit-table conversion")
            except (ValueError, TypeError) as e:
                logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        # マージされたフリーフォームテキストエントリを行番号の昇順で出力。
        last_emitted_row = None
        if merged_texts:
            logger.debug(f"[DEBUG] merged_texts出力開始: {len(merged_texts)}件")
            for (r, txt, is_excl_end) in merged_texts:
                logger.debug(f"[DEBUG] merged_texts出力: 行{r}, text='{txt[:50]}...' (is_excl_end={is_excl_end})")
    ...
    def _detect_and_process_plain_text_regions(self, sheet, min_row: int, max_row: int, min_col: int, max_col: int, processed_rows: set = None) -> set:
        ...
        for row_num in range(min_row, max_row + 1):
            ...
            if self._is_plain_text_region(sheet, region):
                logger.debug(f"[DEBUG] プレーンテキスト行を検出: 行{row_num}")
                # 連続するプレーンテキスト行を検索
    ...
    def _process_excluded_region_as_text(self, sheet, region: Tuple[int, int, int, int]):
        ...
        if end_row >= start_row:  # 何らかのテキストが処理された場合
            ...
            if getattr(self, '_in_canonical_emit', False):
                ...
                logger.debug(f"[TRACE] Skipping authoritative mapping for excluded_region rows {start_row}-{end_row} (non-canonical)")
        ...
        if text_content:
            ...
            if getattr(self, '_in_canonical_emit', False):
                ...
                try:
                    ...
                    pass  # XML解析エラーは無視
            logger.debug(f"[DEBUG] プレーンテキスト出力: {len(text_content)}行")
...
def convert_xls_to_xlsx(xls_file_path: str) -> Optional[str]:
    ...
    try:
        ...
        if not os.path.exists(xlsx_output_path):
            logger.error(f"変換後のXLSXファイルが見つかりません: {xlsx_output_path}")
            shutil.rmtree(temp_dir)
            return None
        
        logger.info(f"XLS→XLSX変換完了: {xlsx_output_path}")
...
def main():
    ...
    if not os.path.exists(args.excel_file):
        logger.debug(f"エラー: ファイル '{args.excel_file}' が見つかりません。")
        sys.exit(1)
    
    if not args.excel_file.endswith(('.xlsx', '.xls')):
        logger.debug("エラー: .xlsxまたは.xls形式のファイルを指定してください。")
        sys.exit(1)
        ...
        if converted_temp_dir:
            try:
                if converted_temp_dir.exists() and converted_temp_dir.name.startswith('xls2md_conversion_'):
                    ...
                    logger.debug(f"一時ディレクトリを削除: {converted_temp_dir}")
            except Exception as cleanup_error:
                logger.debug(f"一時ファイル削除に失敗: {cleanup_error}")
