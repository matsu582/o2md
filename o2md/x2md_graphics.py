...
class _GraphicsMixin:
    ...
    def _process_sheet_images(self, sheet, insert_index: Optional[int] = None, insert_images: bool = True):
        """シート内の画像を処理"""
        try:
            logger.debug(f"[DEBUG][_process_sheet_images_entry] sheet={sheet.title} insert_index={insert_index} insert_images={insert_images}")
            logger.debug(f"[DEBUG][_process_sheet_images_entry] sheet={sheet.title} insert_index={insert_index} insert_images={insert_images}")
            ...
            if sheet.title in self._sheet_shapes_generated:
                logger.debug(f"[DEBUG][_process_sheet_images] sheet={sheet.title} shapes already generated; skipping repeated processing")
                return False
            images_found = False
            ...
            if True:  # 描画図形の分離グループ処理を常に実行
                    logger.debug(f"[DEBUG] {len(sheet._images)} 個の埋め込み画像が検出されました。描画要素を調査中...")
                    # 埋め込み画像が1つ（またはゼロ）の場合、
                    ...
                    try:
                        ...
                        if emb_count == 1:
                            logger.debug(f"[DEBUG][_process_sheet_images_shortcircuit] sheet={sheet.title} single embedded image detected; using embedded image without clustering")
                        ...
                        if rels_xml is not None:
                            ...
                            if drawing_target:
                                ...
                                if drawing_xml is not None:
                                    ...
                                    logger.debug(f"[DEBUG] Sheet '{sheet.title}': total_anchors={total_anchors}, pic_anchors={pic_anchors}, sp_anchors={sp_anchors}, sheet._images={len(sheet._images)}")
                                    logger.debug(f"[DEBUG] Sheet '{sheet.title}': total_anchors={total_anchors}, pic_anchors={pic_anchors}, sp_anchors={sp_anchors}, sheet._images={len(sheet._images)}")
                                    
                                    # 埋め込み画像よりアンカーが多く、少なくとも1つの図形がある場合、
                                    # ベクトル図形をキャプチャするためisolated-groupレンダリングを試行
                                    logger.debug(f"[DEBUG] Checking condition: total_anchors({total_anchors}) > len(sheet._images)({len(sheet._images)}) = {total_anchors > len(sheet._images)} AND sp_anchors({sp_anchors}) > 0 = {sp_anchors > 0}")
                                    if total_anchors > len(sheet._images) and sp_anchors > 0:
                                        logger.debug(f"[DEBUG] Condition TRUE - entering isolated group rendering block for sheet '{sheet.title}'")
                                        logger.debug(f"[DEBUG] Detected additional drawing shapes (anchors={total_anchors}, pics={pic_anchors}, sps={sp_anchors}) - attempting isolated-group rendering")
                                        try:
                                            # 図形のバウンディングボックスを抽出
                                            shapes = None
                                            try:
                                                logger.debug(f"[DEBUG] Calling _extract_drawing_shapes for sheet '{sheet.title}'")
                                                shapes = self._extract_drawing_shapes(sheet)
                                                logger.debug(f"[DEBUG] _extract_drawing_shapes returned {len(shapes) if shapes else 0} shapes")
                                            except Exception as shape_ex:
                                                logger.warning(f"_extract_drawing_shapes failed: {shape_ex}")
                                                import traceback
                                                traceback.print_exc()
                                            
                                            logger.debug(f"[DEBUG] _extract_drawing_shapes returned: {len(shapes) if shapes else 'None'} shapes")
                                            logger.debug(f"[DEBUG] Checking shapes: shapes={'Not None' if shapes else 'None'}, len={len(shapes) if shapes else 0}")
                                            if shapes and len(shapes) > 0:
                                                logger.debug(f"[DEBUG] Shapes condition TRUE - entering clustering block")
                                                ...
                                                logger.debug(f"[DEBUG] Calling _cluster_shapes_common with {len(shapes)} shapes")
                                                clusters, debug_info = self._cluster_shapes_common(
                                                    sheet, shapes, cell_ranges=cell_ranges_all, max_groups=1
                                                )
                                                logger.debug(f"[DEBUG] _cluster_shapes_common returned {len(clusters)} clusters")
                                                logger.debug(f"[DEBUG] clustered into {len(clusters)} groups: sizes={[len(c) for c in clusters]}")
                                                logger.debug(f"[DEBUG] clustering debug_info: {debug_info}")
                                                ...
                                                logger.debug(f"[DEBUG] Starting to render {len(clusters)} clusters for sheet '{sheet.title}'")
                                                for idx, cluster in enumerate(clusters):
                                                    if len(cluster) > 0:
                                                        logger.debug(f"[DEBUG] Rendering cluster {idx+1}/{len(clusters)} with {len(cluster)} shapes")
                                                        result = self._render_sheet_isolated_group(sheet, cluster)
                                                        logger.debug(f"[DEBUG] Cluster {idx+1} rendering result: {result}")
                                                        if result:
                                                            ...
                                                            if hasattr(self, '_last_iso_preserved_ids') and self._last_iso_preserved_ids:
                                                                ...
                                                                logger.debug(f"[DEBUG] クラスタ {idx+1} の図形ID: {len(cluster_shape_ids)} shapes")
                                                            
                                                            isolated_images.append((cluster_row, img_name, cluster_shape_ids))
                                                            logger.info(f"シート '{sheet.title}' のクラスタ {idx+1} をisolated groupとして出力: {img_name} (row={cluster_row})")
                                                
                                                if isolated_produced:
                                                    logger.info(f"シート '{sheet.title}' の図形をisolated groupとして出力しました")
                                                    logger.debug(f"[DEBUG] isolated_images count: {len(isolated_images)}")
                                                    ...
                                                    for cluster_row, img_name, cluster_shape_ids in isolated_images:
                                                        logger.debug(f"[DEBUG] Processing isolated group image: {img_name} at row={cluster_row}")
                                                        try:
                                                            self._mark_image_emitted(img_name)
                                                            logger.debug(f"[DEBUG] _mark_image_emitted succeeded for: {img_name}")
                                                        except Exception as e:
                                                            logger.warning(f"_mark_image_emitted failed: {e}")
                                                        ...
                                                        try:
                                                            ...
                                                            logger.debug(f"[DEBUG] isolated group画像を_sheet_shape_imagesに追加: {img_name} at row={cluster_row}")
                                                            
                                                            # 各クラスタの図形IDを使用（レンダリング時に保存したもの）
                                                            if cluster_shape_ids:
                                                                self._image_shape_ids[img_name] = cluster_shape_ids
                                                                logger.debug(f"[DEBUG] 図形IDマッピングを保存: {img_name} -> {len(cluster_shape_ids)} shapes")
                                                        except Exception as e:
                                                            logger.warning(f"Failed to add to _sheet_shape_images: {e}")
                                                            import traceback
                                                            traceback.print_exc()
                                            else:
                                                isolated_produced = False
                                        except Exception as e:
                                            logger.warning(f"isolated-group rendering failed: {e}")
                                            import traceback
                                            traceback.print_exc()
                                            isolated_produced = False
            ...
            if hasattr(sheet, '_images') and sheet._images:
                logger.info(f"シート '{sheet.title}' 内の画像を処理中...")
                images_found = True
                            ...
                            if getattr(self, '_in_canonical_emit', False):
                                ...
                                try:
                                    ...
                                    if mapped_cid and str(mapped_cid) in global_iso_preserved_ids:
                                        logger.debug(f"[DEBUG][_emit_image_skip] sheet={sheet.title} embedded image {img_name} suppressed (cid={mapped_cid} already preserved)")
                                        continue
                                except (OSError, IOError, FileNotFoundError):
                                    logger.warning(f"ファイル操作エラー: {e if 'e' in locals() else '不明'}")
                                ...
                                try:
                                    ...
                                    global_iso_preserved_ids = getattr(self, '_global_iso_preserved_ids', set()) or set()
                                    if mapped_cid and str(mapped_cid) in global_iso_preserved_ids:
                                        logger.debug(f"[DEBUG][_defer_image_skip] sheet={sheet.title} embedded image {img_name} suppressed on defer (cid={mapped_cid} already preserved)")
                                    else:
                                        self._sheet_shape_images.setdefault(sheet.title, [])
                                        self._sheet_shape_images[sheet.title].append((start_row, img_name))
                                except (ValueError, TypeError) as e:
                                ...
                                try:
                                    ...
                                    global_iso_preserved_ids = getattr(self, '_global_iso_preserved_ids', set()) or set()
                                    if mapped_cid and str(mapped_cid) in global_iso_preserved_ids:
                                        logger.debug(f"[DEBUG][_noncanonical_image_skip] sheet={sheet.title} embedded image {img_name} suppressed (cid={mapped_cid} already preserved)")
                                        continue
            ...
            if not images_found:
                logger.debug(f"[DEBUG] イメージが見つかりませんでした。")
                ...
                # 以前と同様にレンダリングを続行します。
                if shapes == []:
                    logger.info(f"シート '{sheet.title}' に描画要素が見つかりませんでした（XML解析結果）。フォールバックレンダリングをスキップします。")
                    return False

                logger.info(f"シート '{sheet.title}' に検出されたラスタ画像がありません。フォールバックレンダリングを試行します...")
                try:
                    ...
                    if rendered:
                        ...
                        try:
                            ...
                            if imgs:
                                ...
                                if hasattr(self, '_sheet_shape_image_start_rows') and self._sheet_shape_image_start_rows.get(sheet.title):
                                    ...
                                    try:
                                        # 診断用にログ出力するが使用しない
                                        logger.debug(f"[DEBUG] Ignoring persisted start_map for sheet={sheet.title}")
                                    except (ValueError, TypeError) as e:
                                        logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                                ...
                                try:
                                    logger.debug(f"[DEBUG][_img_insertion_debug] sheet={sheet.title} sheet_map={sheet_map}")
                                    logger.debug(f"[DEBUG][_img_insertion_debug] imgs_by_row={imgs_by_row}")
                                    logger.debug(f"[DEBUG][_img_insertion_debug] normalized_pairs={normalized}")
                                except (ValueError, TypeError) as e:
                                    logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                                ...
                                for row_num in sorted(imgs_by_row.keys()):
                                    ...
                                    if sheet_map and md_pos is not None:
                                        ...
                                        if getattr(self, '_in_canonical_emit', False):
                                            ...
                                            logger.debug(f"[TRACE] Skipping sheet_map offset updates in non-canonical pass for sheet={sheet.title}")
                                ...
                                try:
                                        if md_index_map:
                                            logger.info(f"[_final_img_map] sheet={sheet.title} insert_mappings={md_index_map}")
                                except (ValueError, TypeError) as e:
                                    logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                        except (ValueError, TypeError):
                        ...
                        logger.warning(f"フォールバックレンダリングが実行されませんでした（外部ツール未検出など）。")
                except Exception as e:
                    logger.warning(f"フォールバックレンダリング中にエラーが発生しました: {e}")
        ...
        try:
            ...
            if hasattr(image, '_data') and callable(getattr(image, '_data')):
                try:
                    ...
                    logger.debug(f"[DEBUG] image._data() succeeded for image #{self.image_counter} on sheet '{sheet_name}'")
                except ValueError:
            ...
            if image_data is None:
                ...
                try:
                    ...
                    logger.debug(f"[DEBUG] image.ref-based extraction succeeded for image #{self.image_counter} on sheet '{sheet_name}'")
                except (ValueError, TypeError):
                    image_data = None

            if not image_data:
                logger.warning("画像データを取得できませんでした")
                return
                ...
                if os.path.exists(image_path):
                    ...
                    try:
                        ...
                        if existing == image_data:
                            # 再利用
                            logger.debug(f"[DEBUG] 既存の画像ファイルを再利用: {image_filename}")
                        else:
    ...
    def _deduplicate_image_files(self):
        ...
        try:
            ...
            try:
                import hashlib as _hashlib
                logger.debug('[DEBUG][_dedupe] listing images and computing sha256 before dedupe:')
                    ...
                    try:
                        ...
                        with open(_fp, 'rb') as _f:
                            for _chunk in iter(lambda: _f.read(8192), b''):
                                _h.update(_chunk)
                        logger.debug(f"[DEBUG][_dedupe] pre-sha {_fn} = {_h.hexdigest()}")
                    except Exception as _e:
                        logger.debug(f"[DEBUG][_dedupe] pre-sha {_fn} FAILED: {_e}")
            except (OSError, IOError, FileNotFoundError):
                # 致命的ではない。通常の重複排除を続行
                pass
            ...
            for h, items in groups.items():
                ...
                try:
                    ...
                    if len(bases) != 1 or (self.base_name not in bases):
                        logger.debug(f"[DEBUG][_dedupe] skipping cross-workbook dedupe for hash {h}: bases={bases}")
                        # このグループのファイルは削除しない。そのまま残す
                        continue
                except (ValueError, TypeError):
                    # 発生元の判定に失敗した場合、保守的にスキップ
                    logger.debug(f"[DEBUG][_dedupe] skipping dedupe for hash {h} due to error determining origins")
                    continue
                ...
                for dup in duplicate_names:
                    try:
                        ...
                        if os.path.exists(p):
                            os.remove(p)
                            logger.debug(f"[DEBUG][_dedupe] removed duplicate image: {dup} -> canonical: {canonical}")
                    except (ValueError, TypeError):
                        pass  # ファイル操作失敗は無視
    ...
    def _get_drawing_xml_and_metadata(self, sheet) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Drawing XML取得失敗: {e}")
            try:
                z.close()
            except:
                pass  # データ構造操作失敗は無視
    ...
    def _resolve_connector_references(
        ...
        logger.debug(f"[DEBUG][_resolve_connector] keep={len(keep_cnvpr_ids)} → preserve={len(preserve)} (rows={sorted(list(group_rows))})")
        return preserve
            ...
            for node in list(root):
                ...
                if lname in ('twocellanchor', 'onecellanchor'):
                    ...
                    if this_cid is not None and str(this_cid) in keep_cnvpr_ids:
                        kept_count += 1
                        logger.debug(f"[DEBUG][_prune] KEEP anchor id={this_cid}")
                        continue
                    ...
                    try:
                        root.remove(node)
                        removed_count += 1
                        logger.debug(f"[DEBUG][_prune] REMOVE anchor id={this_cid}")
                    except (ValueError, TypeError):
                        try:
                            root.remove(node)
                            removed_count += 1
                            logger.debug(f"[DEBUG][_prune] REMOVE anchor id={this_cid} (retry)")
                        except (ValueError, TypeError) as e:
                            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
            
            logger.debug(f"[DEBUG][_prune] Summary: kept={kept_count}, removed={removed_count}, total={kept_count+removed_count}")
            tree.write(drawing_relpath, encoding='utf-8', xml_declaration=True)
        except Exception as e:
            logger.debug(f"[DEBUG][_prune_drawing_anchors] Error: {e}")
    ...
    def _convert_excel_to_pdf(self, xlsx_path: str, tmpdir: str, apply_fit_to_page: bool = True) -> Optional[str]:
        ...
        if not is_libreoffice_available():
            logger.debug("[DEBUG] LibreOfficeが利用できないため、Excel→PDF変換をスキップします")
            return None
        try:
            ...
            logger.debug(f"[DEBUG] LibreOffice export command: {' '.join(cmd)}")
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
            
            if proc.returncode != 0:
                logger.warning(f"LibreOffice PDF 変換失敗: {proc.stderr}")
                return None
            ...
            if not os.path.exists(pdf_path):
                ...
                if not candidates:
                    logger.warning("LibreOffice がPDFを出力しませんでした")
                    return None
            ...
            logger.warning(f"Excel→PDF変換失敗: {e}")
            return None
    ...
    def _convert_pdf_page_to_png(self, pdf_path: str, page_index: int, dpi: int,
        ...
        try:
            ...
            logger.debug(f"[DEBUG] PyMuPDFでPDF→PNG変換実行 (ページ {page_index}, DPI: {dpi})...")
            
            doc = fitz.open(pdf_path)
            if page_index >= len(doc):
                logger.warning(f"ページ{page_index}が存在しません（全{len(doc)}ページ）")
                doc.close()
                return None
            ...
            logger.info(f"PNG変換完了: {png_path} (サイズ: {img.size[0]}x{img.size[1]})")
            
            return png_filename
        
        except Exception as e:
            logger.warning(f"PDF→PNG変換失敗: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _convert_excel_to_svg(self, xlsx_path: str, tmpdir: str, apply_fit_to_page: bool = True) -> Optional[str]:
        ...
        """
        if not is_libreoffice_available():
            logger.debug("[DEBUG] LibreOfficeが利用できないため、Excel→SVG変換をスキップします")
            return None
        try:
            # 元のファイルを上書きしないように一時コピーを作成
            tmp_xlsx = os.path.join(tmpdir, os.path.basename(xlsx_path))
            ...
            logger.debug(f"[DEBUG] LibreOffice SVG export command: {' '.join(cmd)}")
            proc = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
            
            if proc.returncode != 0:
                logger.warning(f"LibreOffice SVG 変換失敗: {proc.stderr}")
                return None
            ...
            if not os.path.exists(svg_path):
                ...
                if not candidates:
                    logger.warning("LibreOffice がSVGを出力しませんでした")
                    return None
            ...
            logger.warning(f"Excel→SVG変換失敗: {e}")
            return None
    ...
    def _convert_pdf_page_to_svg(self, pdf_path: str, page_index: int,
        ...
        try:
            ...
            logger.debug(f"[DEBUG] PyMuPDFでPDF→SVG変換実行 (ページ {page_index})...")
            ...
            logger.info(f"SVG変換完了: {svg_path}")
            
            return svg_filename
        
        except Exception as e:
            logger.warning(f"PDF→SVG変換失敗: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _render_sheet_fallback(self, sheet, dpi: int = 600, insert_index: Optional[int] = None, insert_images: bool = True) -> bool:
        ...
        try:
            ...
            # 1. Excel→PDF変換
            logger.debug(f"[DEBUG] Fallback rendering for sheet: {sheet.title}")
            pdf_path = self._convert_excel_to_pdf(self.excel_file, tmpdir, apply_fit_to_page=True)
            if pdf_path is None:
                logger.warning("Excel→PDF変換に失敗しました（LibreOfficeが利用できないか、変換エラー）")
                return False
            ...
            if insert_images:
                ...
                try:
                    ...
                    logger.info(f"シート全体の画像を挿入: {result_filename}")
                except Exception as e:
                    logger.warning(f"画像挿入失敗: {e}")
            ...
            logger.warning(f"フォールバックレンダリングエラー: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _extract_drawing_cell_ranges(self, sheet) -> List[Tuple[int,int,int,int]]:
        ...
        """
        logger.info(f"シート図形セル範囲抽出: {sheet.title}")
        ranges = []
        try:
            ...
            pass  # データ構造操作失敗は無視
        logger.info(f"抽出されたセル範囲: {ranges}")
        return ranges
    ...
    def _extract_shape_metadata_from_anchor(self, anchor, sheet) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Shape metadata extraction failed: {e}")
            return metadata
    ...
    def _extract_all_shapes_metadata(self, sheet, filter_ids: Optional[Set[str]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Failed to extract shapes metadata: {e}")
    ...
    def _anchor_has_drawable(self, a) -> bool:
        ...
        try:
            ...
            result = False
            if drawable_types:
                logger.debug(f"[DEBUG] Anchor has drawable elements: {drawable_types}")
                result = True
            elif has_connector_ref:
                logger.debug(f"[DEBUG] Anchor has connector references; treating as drawable")
                result = True
            elif has_text:
                logger.debug(f"[DEBUG] Anchor contains only text; treating as non-drawable")
                result = False
            else:
                logger.debug(f"[DEBUG] Anchor has no drawable elements")
                result = False
    ...
    def _cluster_shapes_common(self, sheet, shapes, cell_ranges=None, max_groups=2):
        ...
        try:
            ...
            logger.debug(f"[DEBUG] クラスタリングエラー: {e}")
    ...
    def _extract_drawing_shapes(self, sheet) -> List[Tuple[int,int,int,int]]:
        ...
        try:
            ...
            logger.debug(f"[DEBUG] _extract_drawing_shapes found {len(bboxes)} bboxes")
            return bboxes
        except Exception as e:
            logger.warning(f"_extract_drawing_shapes exception: {e}")
            import traceback
            traceback.print_exc()
            return []
        ...
        **RECOMMENDATION**: Use the original _render_sheet_isolated_group method for production.
        ...
        try:
            ...
            with zipfile.ZipFile(zpath, 'r') as z:
                ...
                rels_xml = get_xml_from_zip(z, rels_path)
                if rels_xml is None:
                    logger.debug(f"[DEBUG][_iso_v2] sheet={sheet.title} missing rels")
                    return None
                drawing_target = None
                for rel in rels_xml.findall('.//{http://schemas.openxmlformats.org/package/2006/relationships}Relationship'):
                    if rel.attrib.get('Type', '').endswith('/drawing'):
                ...
                if not drawing_target:
                    logger.debug(f"[DEBUG][_iso_v2] sheet={sheet.title} no drawing relationship")
                    return None
                ...
                if drawing_path not in z.namelist():
                    drawing_path = drawing_path.replace('worksheets', 'drawings')
                    if drawing_path not in z.namelist():
                        logger.debug(f"[DEBUG][_iso_v2] drawing_path not found: {drawing_path}")
                        return None
            ...
            if not anchors:
                logger.debug(f"[DEBUG][_iso_v2] no drawable anchors found")
                return None
            ...
            if cell_range is None and shape_indices:
                try:
                    ...
                    if picked:
                        ...
                        logger.debug(f"[DEBUG][_iso_v2] Computed cell_range from shapes: cols {s_col}-{e_col}, rows {s_row}-{e_row} (with padding)")
                        logger.debug(f"[DEBUG][_iso_v2] Original shape ranges: {picked}")
                except Exception as e:
                    logger.debug(f"[DEBUG][_iso_v2] Failed to compute cell_range: {e}")            # shape_indicesからkeep_cnvpr_idsを構築
            keep_cnvpr_ids = set()
            ...
            logger.debug(f"[DEBUG][_iso_v2] anchors={len(anchors)} keep_ids={sorted(list(keep_cnvpr_ids))}")
            ...
            try:
                ...
                try:
                    ...
                    if os.path.exists(wb_path):
                        ...
                        if sheets_el is not None:
                            ...
                            logger.debug(f"[DEBUG][_iso_v2] Removed {len(sheets_to_remove)} non-target sheets from workbook (kept drawing: {target_sheet_drawing or 'none'})")
                ...
                logger.debug(f"[DEBUG][_iso_v2] Preserving original drawing coordinates (no adjustment)")
                if cell_range:
                    s_col, e_col, s_row, e_row = cell_range
                    logger.debug(f"[DEBUG][_iso_v2] Cell range for data trimming: cols {s_col}-{e_col}, rows {s_row}-{e_row}")
                ...
                if os.path.exists(sheet_rel):
                    try:
                        ...
                        logger.debug(f"[DEBUG][_iso_v2] Set pageSetup to scale=100 (normal size) to preserve shapes")
                    except Exception as e:
                        if getattr(self, 'verbose', False):
                ...
                if False and os.path.exists(sheet_rel) and cell_range:
                    try:
                        ...
                        if sheet_data is not None and src_sheet_data is not None:
                            ...
                            logger.debug(f"[DEBUG][_iso_v2] Found {len(src_rows)} rows in source sheet.xml")
                            cells_copied = 0
                            ...
                            logger.debug(f"[DEBUG][_iso_v2] Copied {cells_copied} cells with original row/col numbers")
                        ...
                        stree.write(sheet_rel, encoding='utf-8', xml_declaration=True)
                        logger.debug(f"[DEBUG][_iso_v2] Reconstructed sheet data: kept original rows {s_row}-{e_row}, cols {s_col}-{e_col}")
                    except Exception as e:
                        if getattr(self, 'verbose', False):
                ...
                try:
                    ...
                    if os.path.exists(wb_rel):
                        ...
                        if dn is not None:
                            wroot.remove(dn)
                            logger.debug(f"[DEBUG][_iso_v2] Removed Print_Area and all defined names to ensure shapes are visible")
                ...
                try:
                    ...
                    logger.debug(f"[DEBUG][_iso_v2] Saved debug workbook: {debug_xlsx_path}")
                except Exception as e:
                    if getattr(self, 'verbose', False):
                ...
                try:
                    if not is_libreoffice_available():
                        logger.debug("[DEBUG][_iso_v2] LibreOfficeが利用できないため、図形レンダリングをスキップします")
                        return None
                    ...
                    logger.debug(f"[DEBUG][_iso_v2] LibreOffice command: {' '.join(cmd)}")
                    proc = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
                    ...
                    logger.debug(f"[DEBUG][_iso_v2] Saved debug PDF: {pdf_path}")
                    ...
                    logger.debug(f"[DEBUG][_iso_v2] ImageMagick command: {' '.join(cmd)}")
                    ...
                    try:
                        ...
                        if os.path.exists(png_path):
                            ...
                            if bbox:
                                ...
                                cropped.close()
                                logger.debug(f"[DEBUG][_iso_v2] Cropped image: {im.size} → {cropped.size}")
                            im.close()
                    ...
                    logger.debug(f"[DEBUG][_iso_v2] Returning: filename={png_filename}, cluster_min_row={cluster_min_row}")
                    return (png_filename, cluster_min_row)
                    
                except Exception as e:
                    if getattr(self, 'verbose', False):
                        logger.error(f"[_iso_v2] Conversion failed: {e}")
                    return None
            ...
            logger.error(f"[_iso_v2] Exception: {e}")
            import traceback
            traceback.print_exc()
            return None
