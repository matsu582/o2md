...
- **PDF変換** (pdf2md.py): PDFを画像とテキストに変換（Tesseract/manga-ocr/sarashina2.2-ocrによるOCR対応）
...
- **OCR対応**: 画像ベースPDF（スキャンPDF）のテキスト抽出
  ...
  - sarashina2.2-ocr: End-to-End VLMによる高精度OCR（GPU推奨）
- **tessdata_best対応**: `--tessdata-dir`オプションで高精度モデルを指定可能
- **docling表検出対応**: `--docling`オプションで罫線のない表も検出可能
  - TableFormerモデルを使用した高精度な表検出
