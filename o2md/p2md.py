...
class PowerPointToMarkdownConverter:
    ...
    def convert(self) -> str:
        ...
        print(_("PowerPoint文書変換開始: {file}").format(file=self.pptx_file))
        ...
        for slide_idx, slide in enumerate(self.prs.slides, 1):
            ...
            print(_("スライド {current}/{total} を処理中...").format(current=slide_idx, total=len(self.prs.slides)))
            
            try:
                self._convert_slide(slide, slide_idx)
            except Exception as e:
                logger.warning(f"スライド {slide_idx} の処理中にエラー: {e}")
                import traceback
                traceback.print_exc()
                continue
    ...
    def _convert_slide(self, slide, slide_idx: int):
        ...
        if is_complex:
            logger.info(f"スライド {slide_idx}: 複合スライド検出 - テキスト/表を展開後、スライド全体を画像化")
        elif has_charts:
            logger.info(f"スライド {slide_idx}: チャート検出 - スライド全体を画像化（チャートデータは画像内に含まれます）")
        ...
        if has_shapes or has_charts:
            if needs_image:
                ...
                logger.info(f"スライド {slide_idx}: スライド全体を画像化")
                self._render_slide_as_image(slide, slide_idx)
            elif has_shapes:
                # 図形のみ：図形群を画像化
                logger.info(f"スライド {slide_idx}: 図形のみ - 図形群を画像化")
    ...
    def _convert_chart_to_markdown(self, chart) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"[DEBUG] チャート変換エラー: {e}")
            return None
    ...
    def _render_slide_as_image(self, slide, slide_idx: int):
        ...
        try:
            ...
            if success:
                ...
                self.markdown_lines.append("")
                logger.info(f"スライド全体画像化: {actual_filename}")
            
        except Exception as e:
            logger.error(f"スライド画像化エラー: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _get_or_create_pdf(self) -> Optional[str]:
        ...
        if not is_libreoffice_available():
            logger.warning("LibreOfficeが利用できないため、スライドの画像レンダリングをスキップします")
            return None
    ...
    def _convert_pdf_page_to_png(self, pdf_path: str, page_index: int, output_path: str) -> bool:
        ...
        """
        try:
            logger.debug(f"[DEBUG] PyMuPDFでPDF→PNG変換実行 (ページ {page_index})...")
            
            doc = fitz.open(pdf_path)
            if page_index >= len(doc):
                logger.error(f"ページ{page_index}が存在しません（全{len(doc)}ページ）")
                doc.close()
                return False
    ...
    def cleanup(self):
        ...
        if self._temp_pptx_file and os.path.exists(self._temp_pptx_file):
            try:
                ...
                logger.info(f"一時pptxファイルを削除: {self._temp_pptx_file}")
            except Exception as e:
                logger.warning(f"一時pptxファイル削除エラー: {e}")
    ...
    def _convert_ppt_to_pptx(self, ppt_file: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"LibreOfficeを使用してppt→pptx変換を実行中...")
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=180)
            ...
            if result.returncode == 0:
                ...
                for file in os.listdir(temp_dir):
                    if file.endswith('.pptx'):
                        ...
                        logger.info(f"ppt→pptx変換完了: {pptx_path}")
                        return pptx_path
                
                logger.error(f"pptx変換失敗: 出力ファイルが見つかりません")
                shutil.rmtree(temp_dir)
                return None
            else:
                logger.error(f"pptx変換失敗: {result.stderr}")
                shutil.rmtree(temp_dir)
                return None
                
        except Exception as e:
            logger.error(f"pptx変換エラー: {e}")
            import traceback
            traceback.print_exc()
            return None
...
def main():
    ...
    if not os.path.exists(args.pptx_file):
        print(_("エラー: ファイル '{file}' が見つかりません。").format(file=args.pptx_file))
        sys.exit(1)
    
    if not (args.pptx_file.lower().endswith('.pptx') or args.pptx_file.lower().endswith('.ppt')):
        print(_("エラー: .pptまたは.pptx形式のファイルを指定してください。"))
        sys.exit(1)
