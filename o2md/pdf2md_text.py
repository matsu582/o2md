...
class _TextMixin:
    ...
    def _extract_structured_text(self, page) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"[DEBUG] テキスト抽出エラー: {e}")
            return []
            ...
            for line in block.get("lines", []):
                ...
                for line_spans in span_groups:
                    ...
                    if self._is_header_footer(
                        ...
                        logger.debug(f"[DEBUG] ヘッダ・フッタ除外: {line_text.strip()[:30]}...")
                        continue
                    ...
                    if is_centered and is_single_digit:
                        logger.debug(f"[DEBUG] 装飾的テキスト除外: '{line_text_stripped}' at x={line_center_x:.1f}")
                        continue
                    ...
                    if not is_caption:
                        for fig_bbox in exclude_bboxes:
                            if (fig_bbox[0] <= line_center_x <= fig_bbox[2] and
                                ...
                                break
                    if in_figure:
                        logger.debug(f"[DEBUG] 図領域内テキスト除外: {line_text_stripped[:30]}...")
                        continue
                    ...
                    if line2_spans:
                        ...
                        if (first_span_size < base_font_size * 0.7 and
                            ...
                            if abs(line1_right - line2_left) < 10:
                                ...
                                logger.debug(f"[DEBUG] 傍注結合: {line1['text'][:20]}... + <sup>{sup_text}</sup>")
                                break
        ...
        for group in y_groups:
            ...
            for i in range(1, len(group_sorted)):
                ...
                if x_gap <= font_size * 1.5 and not is_cross_column:
                    ...
                    logger.debug(f"[DEBUG] 同一Y行結合: {prev['text'][:20]}... + {curr['text'][:20]}...")
                else:
    ...
    def _sort_lines_by_column(
        ...
        def cluster_and_sort_by_row(lines: List[Dict], skip_clustering: bool = False) -> List[Dict]:
            ...
            if skip_clustering:
                logger.debug("[DEBUG] スライド文書: 行クラスタリングをスキップ")
                return sorted_by_y
            ...
            for i, line in enumerate(lines):
                ...
                if number_only_pattern.match(text) and i + 1 < len(lines):
                    ...
                    if (y_diff <= threshold and 
                        ...
                        skip_next = True
                        logger.debug(f"[DEBUG] 番号+タイトルをマージ: '{text}' + '{next_text}' -> '{merged_text}'")
                        continue
                ...
                if i + 1 < len(full_lines):
                    ...
                    if page_fraction_pattern.match(next_text):
                        ...
                        if re.match(r'^[０-９\d]+[．.]\s*', text):
                            ...
                            skip_next = True
                            logger.debug(f"[DEBUG] スライドタイトル+ページ番号をマージ: '{text}' + '{next_text}'")
                            continue
    ...
    def _reflow_paragraphs_with_tables(
            ...
            for fig_bbox in figure_bboxes:
                ...
                if overlap_x0 < overlap_x1 and overlap_y0 < overlap_y1:
                    ...
                    if overlap_area / table_area >= 0.8:
                        logger.debug(f"[DEBUG] テーブルが図形と重なるためスキップ: table_bbox={table_bbox}, fig_bbox={fig_bbox}")
                        return True
        ...
        if is_slide_document:
            logger.debug("[DEBUG] スライド文書: 段落結合をスキップ")
            blocks = []
            for line in lines:
                ...
                if page_width and page_height and text.isdigit() and len(text) <= 3:
                    ...
                    if is_at_bottom and is_at_edge and is_narrow:
                        logger.debug(f"[DEBUG] ページ番号を除外: text=\"{text}\", y={y_top:.1f}, x_center={x_center:.1f}")
                        continue
        ...
        if is_landscape:
            ...
            logger.debug(f"[DEBUG] 横長ページ検出: 改行保持モード（gap_threshold={gap_threshold:.1f}）")
        else:
            gap_threshold = line_height * 0.8
