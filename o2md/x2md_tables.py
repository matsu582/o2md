...
class _TablesMixin:
            ...
            for col in range(min_col, max_col + 1):
                ...
                if (cell.border and cell.border.left and cell.border.left.style and
                    ...
                    if region:
                        r1, r2, c1, c2 = region
                        if r1 == 1 and r2 <= 4:
                            logger.debug(f"[DEBUG][{sheet.title}] Top region detected: rows {r1}-{r2}, cols {c1}-{c2}")
                        if r2 - r1 >= 1 or c2 - c1 >= 2:
                            is_valid = self._is_valid_bordered_table(sheet, region)
                            if r1 == 1 and r2 <= 4:
                                logger.debug(f"[DEBUG][{sheet.title}] Top region validation: {is_valid}")
                            if is_valid:
                                tables.append(region)
        
        logger.debug(f"[DEBUG] _detect_bordered_tables found {len(tables)} tables")
        return tables
    ...
    def _find_discrete_data_regions(self, sheet, min_row: int, max_row: int, min_col: int, max_col: int, occupied_cells: Optional[Set[Tuple[int, int]]] = None) -> List[Tuple[int, int, int, int]]:
        ...
        """
        logger.debug(f"[DEBUG][_find_discrete_data_regions] sheet={sheet.title} range=({min_row}-{max_row}, {min_col}-{max_col}) occupied_cells_count={len(occupied_cells) if occupied_cells else 0}")
                ...
                if table_bounds:
                    ...
                    if c2 - c1 >= 1:  # 2列以上の場合のみテーブルとして追加
                        ...
                        if total_rows > 0 and multi_col_rows / total_rows >= 0.3:
                            ...
                            logger.debug(f"[DEBUG][_find_discrete_data_regions] accepted region {table_bounds}: {multi_col_rows}/{total_rows} rows have 2+ cols")
                        else:
                            logger.debug(f"[DEBUG][_find_discrete_data_regions] rejected region {table_bounds}: only {multi_col_rows}/{total_rows} rows have 2+ cols")
                    else:
                        logger.debug(f"[DEBUG][_find_discrete_data_regions] rejected single-column region {table_bounds}")
                    visited.update(visited_cells)
        
        logger.debug(f"[DEBUG][_find_discrete_data_regions] found {len(tables)} discrete regions: {tables[:5]}")
        return tables
    ...
    def _is_valid_bordered_table(self, sheet, region):
        ...
        logger.debug(f"[DEBUG] Table region {region}: empty_rows={empty_rows}/{total_rows} (ratio={empty_row_ratio:.2f}), empty_cols={empty_cols}/{total_cols} (ratio={empty_col_ratio:.2f}), cols_with_data={cols_with_data}")
        ...
        if has_many_merged_cells and cols_with_data >= 3 and is_small_table:
            logger.debug(f"[DEBUG] Small table with many merged cells ({len(merged_cols)} cols, {len(merged_rows)} rows) and {cols_with_data} columns with data, relaxing empty column threshold")
            result = empty_row_ratio < 0.5 and empty_col_ratio < 0.8
            if r1 == 1 and r2 <= 4:
                logger.debug(f"[DEBUG][{sheet.title}] Top region validation details: has_many_merged_cells={has_many_merged_cells}, is_small_table={is_small_table}, result={result}")
            return result
    ...
    def _is_colon_separated_list(self, sheet, start_row: int, end_row: int, start_col: int, end_col: int) -> bool:
        ...
        if total_data_rows > 0 and (rows_with_colon / total_data_rows) >= 0.5:
            logger.debug(f"[DEBUG] コロン区切りリスト検出: {rows_with_colon}/{total_data_rows}行がコロンを含む")
            return True
    ...
    def _detect_table_regions(self, sheet, min_row: int, max_row: int, min_col: int, max_col: int) -> Tuple[List[Tuple[int, int, int, int]], List[str]]:
        ...
        logger.debug(f"[DEBUG][_detect_table_regions_entry] sheet={getattr(sheet,'title',None)} min_row={min_row} max_row={max_row} min_col={min_col} max_col={max_col}")
        # デバッグ: 基本的なシートメトリクス
        logger.debug(f"[DEBUG][_detect_table_regions_entry] sheet={sheet.title} rows={min_row}-{max_row} cols={min_col}-{max_col} max_row={sheet.max_row} max_col={sheet.max_column}")
    ...
    def _split_horizontal_tables_with_annotations(self, sheet, table_regions: List[Tuple[int, int, int, int]]) -> Tuple[List[Tuple[int, int, int, int]], List[str]]:
        ...
        for region in table_regions:
            ...
            logger.debug(f"[DEBUG] 分離処理中の領域: 行{start_row}〜{end_row}, 列{start_col}〜{end_col}")
            ...
            if (end_col - start_col) < 8:  # 8列未満は分離しない
                logger.debug(f"[DEBUG] 列数が少ないため分離せず: {end_col - start_col + 1}列")
            ...
            if len(clear_boundaries) > 1 and not is_single_table:
                ...
                logger.debug(f"[DEBUG] 罫線境界を直接使用: {clear_boundaries}")
                ...
                for boundary_start, boundary_end in clear_boundaries:
                    ...
                    if boundary_end - boundary_start == 0:  # 1列
                        ...
                        if not (boundary_start == start_col and boundary_end == end_col):
                            ...
                            logger.debug(f"[DEBUG] 複数列境界テーブル追加: {(start_row, end_row, boundary_start, boundary_end)}")
                        else:
                            logger.debug(f"[DEBUG] 複数列境界は全体と同じためスキップ: {(start_row, end_row, boundary_start, boundary_end)}")
                ...
                if param_boundaries and value_boundaries:
                    ...
                    if param_value_table:
                        ...
                        logger.debug(f"[DEBUG] パラメータ-値テーブル追加: {param_value_table}")
                ...
                for boundary_start, boundary_end in clear_boundaries:
                    if boundary_end - boundary_start == 0 and boundary_start == 3:  # 項目名列
                        ...
                        if cleaned_region:
                            separated_tables.append(cleaned_region)
                            logger.debug(f"[DEBUG] 項目名テーブル追加: {cleaned_region}")
            else:
                # 罫線による分離ができない場合、または単一テーブルの場合は従来の方法
                if is_single_table:
                    logger.debug(f"[DEBUG] 単一テーブル検出、分離スキップ")
                else:
                    main_separations = self._find_major_column_separations(sheet, start_row, end_row, start_col, end_col)
                    logger.debug(f"[DEBUG] 従来方式による分離点: {main_separations}")
            
            logger.debug(f"[DEBUG] 検出された分離点: {main_separations}")
            
            if len(main_separations) == 0:
                # 分離点がない場合、注意書きを除外してそのまま追加
                logger.debug(f"[DEBUG] 分離点なし、そのまま追加")
                cleaned_region = self._clean_annotation_from_region(sheet, region)
                if cleaned_region:
                    separated_tables.append(cleaned_region)
                    logger.debug(f"[DEBUG] 分離なし、テーブル追加: {cleaned_region}")
                else:
                    logger.debug(f"[DEBUG] 分離なし、テーブルは空のためスキップ")
            else:
                # 明確な分離点で分ける
                ...
                for i, sep_col in enumerate(main_separations):
                    logger.debug(f"[DEBUG] 分離処理{i+1}: 列{current_start_col}〜{sep_col-1}")
                    ...
                    if sep_col > current_start_col:  # 最低1列は必要（2列→1列に変更）
                        ...
                        if cleaned_region:
                            ...
                            logger.debug(f"[DEBUG] 分離テーブル{i+1}追加: {enhanced_region}")
                        else:
                            logger.debug(f"[DEBUG] 分離テーブル{i+1}は空のためスキップ")
                    else:
                        logger.debug(f"[DEBUG] 分離テーブル{i+1}は列数不足のためスキップ: {sep_col - current_start_col + 1}")
                    current_start_col = sep_col + 1
                
                # 最後の部分
                logger.debug(f"[DEBUG] 最後の部分処理: 列{current_start_col}〜{end_col}")
                # 列数制限を緩和：1列でも有効
                if end_col >= current_start_col:  # 最低1列は必要（3列→1列に変更）
                    ...
                    if cleaned_region:
                        ...
                        logger.debug(f"[DEBUG] 最後のテーブル追加: {final_region}")
                    else:
                        logger.debug(f"[DEBUG] 最後のテーブルは空のためスキップ")
                else:
                    logger.debug(f"[DEBUG] 最後のテーブルは列数不足のためスキップ: 列幅{end_col - current_start_col + 1}")
        ...
        for i, table1 in enumerate(unique_tables):
            ...
            for j, table2 in enumerate(unique_tables):
                ...
                if (r1_start == r2_start and r1_end == r2_end and
                    ...
                    is_subset = True
                    logger.debug(f"[DEBUG] 部分テーブルを除外: {table1} (完全版: {table2})")
                    break
            if not is_subset:
                filtered_tables.append(table1)
        
        logger.debug(f"[DEBUG] 分離結果: {len(filtered_tables)}個のテーブル（重複・部分除去後）")
        for i, table in enumerate(filtered_tables):
            logger.debug(f"[DEBUG] テーブル{i+1}: {table}")
    ...
    def _create_parameter_value_table(self, sheet, start_row: int, end_row: int, param_col: int, value_col: int) -> Optional[Tuple[int, int, int, int]]:
        ...
        """
        logger.debug(f"[DEBUG] パラメータ-値テーブル作成: 行{start_row}〜{end_row}, パラメータ列{param_col}, 値列{value_col}")
        ...
        for row in range(start_row, end_row + 1):
            ...
            if param_value and not self._is_annotation_text(param_value):
                ...
                logger.debug(f"[DEBUG] パラメータ-値ペア: {param_value} → {value_value}")
    ...
    def _detect_table_boundaries_by_clear_borders(self, sheet, min_row: int, max_row: int, min_col: int, max_col: int) -> List[Tuple[int, int]]:
        ...
        """
        logger.debug(f"[DEBUG] 明確な罫線境界検出: 行{min_row}〜{max_row}, 列{min_col}〜{max_col}")
        ...
        for col, strengths in border_strengths.items():
            if strengths['right'] >= 0.95:
                ...
                logger.debug(f"[DEBUG] 強い右側境界線: 列{col} ({strengths['right_count']}/{total_rows}行)")
            elif strengths['right'] >= 0.60:
                moderate_boundaries.append(col)
                logger.debug(f"[DEBUG] 中程度の右側境界線: 列{col} ({strengths['right_count']}/{total_rows}行)")

            if strengths['left'] >= 0.95:
                strong_left_boundaries.append(col)
                logger.debug(f"[DEBUG] 強い左側境界線: 列{col} ({strengths['left_count']}/{total_rows}行)")
            elif strengths['left'] >= 0.60:
                if col not in moderate_boundaries:
                    moderate_boundaries.append(col)
                    logger.debug(f"[DEBUG] 中程度の左側境界線: 列{col} ({strengths['left_count']}/{total_rows}行)")
        ...
        if total_strong_boundaries >= total_columns * 0.9:
            ...
            logger.debug(f"[DEBUG] 格子状テーブル検出（境界線密度高）: 列{min_col}〜{max_col}")
            ...
            for i, start_col in enumerate(table_starts):
                ...
                if end_col >= start_col:
                    ...
                    logger.debug(f"[DEBUG] テーブル境界決定: 列{start_col}〜{end_col}")
        else:
        ...
        logger.debug(f"[DEBUG] 最終境界: {boundaries}")
        return boundaries
    ...
    def _optimize_table_for_two_columns(self, sheet, region: Tuple[int, int, int, int], headers: List[str], header_positions: List[int]) -> Optional[List[List[str]]]:
        ...
        logger.debug(f"[DEBUG] _optimize_table_for_two_columns: headers={headers}, len={len(headers)}, header_positions={len(header_positions)}")

        # ガード: ヘッダー数が3でない場合は最適化をスキップ
        # (正規化後のヘッダー数で判定、元の列数ではない)
        if len(headers) != 3:
            logger.debug(f"[DEBUG] 2列最適化スキップ（ヘッダー数が3ではない: {len(headers)}列）")
            return None
        
        # header_positionsは3つ以上必要
        if len(header_positions) < 3:
            logger.debug(f"[DEBUG] 2列最適化スキップ（ヘッダー位置が3未満: {len(header_positions)}位置）")
            return None
            
        # 3列で、1列目が冗長な場合を検出
        # 第1列と第2列の組み合わせが設定項目のパターンかチェック(名前|初期値のパターン)
        logger.debug(f"[DEBUG] 3列テーブル検出、パターンチェック: '{headers[0]}' と '{headers[1]}' (列{header_positions[0]}, 列{header_positions[1]})")
        ...
        if self._is_setting_item_pattern_columns(sheet, region, header_positions[0], header_positions[1]):
            ...
            if total_rows > 0:
                ...
                if col0_density >= 0.5:
                    logger.debug(f"[DEBUG] 2列最適化 (第1列保持, 密度={col0_density:.1%}): {headers[0]} | {headers[2]}")
            ...
            logger.debug(f"[DEBUG] 2列最適化: {headers[1]} | {headers[2]}")
    ...
    def _is_setting_item_pattern_columns(self, sheet, region: Tuple[int, int, int, int], col1: int, col2: int) -> bool:
        ...
        try:
            ...
            logger.debug(f"[DEBUG] _is_setting_item_pattern_columns: col1={col1}({col1_nonempty}個,distinct={col1_distinct},avg_len={avg_len1:.1f}), col2={col2}({col2_nonempty}個,distinct={col2_distinct},avg_len={avg_len2:.1f})")
            ...
            if col1_distinct >= max(2, col2_distinct):
                score += 1
                logger.debug(f"[DEBUG] スコア+1: col1_distinct({col1_distinct}) >= max(2, col2_distinct({col2_distinct}))")
            if avg_len2 <= max(6, avg_len1 * 0.7):
                score += 1
                logger.debug(f"[DEBUG] スコア+1: avg_len2({avg_len2:.1f}) <= max(6, avg_len1*0.7({avg_len1*0.7:.1f}))")
            if col2_nonempty >= max(2, int(total_rows * 0.2)):
                score += 1
                logger.debug(f"[DEBUG] スコア+1: col2_nonempty({col2_nonempty}) >= max(2, total_rows*0.2({int(total_rows*0.2)}))")
            # 値列のユニーク数が非空に対して低い場合、フラグ的な可能性が高い
            if col2_nonempty > 0 and (col2_distinct / col2_nonempty) <= 0.5:
                score += 1
                logger.debug(f"[DEBUG] スコア+1: col2_distinct/col2_nonempty({col2_distinct}/{col2_nonempty}={col2_distinct/col2_nonempty:.2f}) <= 0.5")
            
            logger.debug(f"[DEBUG] 最終スコア: {score} (必要: 3以上)")
            return score >= 3
    ...
    def _process_table_boundary(self, table_boundaries: List, current_start: Optional[int], 
        ...
        if has_data and has_border and current_start is None:
            logger.debug(f"[DEBUG] テーブル開始検出: 行{row_num} (罫線あり)")
            return row_num
        # テーブル内で罫線がある空行は継続
            ...
            if has_vertical_borders:
                logger.debug(f"[DEBUG] テーブル継続: 行{row_num} (空行だが罫線あり)")
                return current_start
            else:
                # 罫線もない空行ならテーブル終了
                logger.debug(f"[DEBUG] テーブル終了検出: 行{row_num} (罫線なし空行)")
        ...
        # 強い罫線(テーブル外枠)でテーブル終了
            ...
            if has_data:
                logger.debug(f"[DEBUG] テーブル継続: 行{row_num} (データあり)")
                return current_start
            ...
            if is_strong_boundary:
                logger.debug(f"[DEBUG] テーブル終了検出: 行{row_num} (強い罫線、データなし)")
                self._finalize_table_region(table_boundaries, current_start, row_num - 1, 
                                          sheet, min_col, max_col)
                return None
            else:
                logger.debug(f"[DEBUG] テーブル継続: 行{row_num} (内部罫線)")
                return current_start
        return current_start
        ...
        for start_row, end_row, start_col, end_col in table_boundaries:
            ...
            for merged_range in sheet.merged_cells.ranges:
                ...
                if (merged_range.max_row >= start_row and merged_range.min_row <= end_row and
                    ...
                    logger.debug(f"[DEBUG] 結合セルによりテーブル領域拡張: 行{merged_range.min_row}〜{merged_range.max_row}, 列{merged_range.min_col}〜{merged_range.max_col}")
            ...
            if (adjusted_start_row, adjusted_end_row, adjusted_start_col, adjusted_end_col) != (start_row, end_row, start_col, end_col):
                logger.debug(f"[DEBUG] テーブル領域調整: 行{start_row}〜{end_row} -> 行{adjusted_start_row}〜{adjusted_end_row}, 列{start_col}〜{end_col} -> 列{adjusted_start_col}〜{adjusted_end_col}")
    ...
    def _finalize_table_region(self, table_boundaries: List, start_row: int, end_row: int,
        ...
        if end_row >= start_row:
            ...
            if actual_col_range:
                ...
                logger.debug(f"[DEBUG] テーブル検出: 行{start_row}〜{end_row}, 列{actual_col_range[0]}〜{actual_col_range[1]}")
    ...
    def _detect_table_columns_by_borders(self, sheet, start_row: int, end_row: int, min_col: int, max_col: int) -> Optional[Tuple[int, int]]:
        ...
        logger.debug(f"[DEBUG] 列範囲検出: 行{start_row}〜{end_row}, 列{min_col}〜{max_col}")
        ...
        for col in range(min_col, max_col + 1):
            ...
            if total_cells > 0 and (border_count / total_cells) >= 0.5:
                table_start_col = col
                logger.debug(f"[DEBUG] テーブル開始列検出: 列{col} (境界線密度: {border_count}/{total_cells})")
                break
        ...
        for col in range(max_col, min_col - 1, -1):  # 逆順でチェック
            ...
            if total_cells > 0 and (border_count / total_cells) >= 0.5:
                table_end_col = col
                logger.debug(f"[DEBUG] テーブル終了列検出: 列{col} (境界線密度: {border_count}/{total_cells})")
                break
        
        if table_start_col is not None and table_end_col is not None and table_start_col <= table_end_col:
            logger.debug(f"[DEBUG] 罫線ベース列範囲: 列{table_start_col}〜{table_end_col}")
 ...
 new_min: int, new_max: int) -> Tuple[int, int]:
    ...
    def _convert_table_region(self, sheet, region: Tuple[int, int, int, int], table_number: int,
        ...
        try:
            logger.debug(f"[DEBUG][_convert_table_region_entry] sheet={getattr(sheet, 'title', None)} region={start_row}-{end_row},{start_col}-{end_col}")
            ...
            for rr in range(start_row, start_row + max_dump):
                ...
                for cc in range(start_col, end_col + 1):
                    try:
                        ...
                        v = None
                    rowvals.append((cc, v))
                logger.debug(f"[DEBUG][_convert_table_region_entry] raw row {rr}: {rowvals}")
        ...
        if end_row - start_row <= 1:
            ...
            if title_text:
                # タイトルのみの小さなテーブルはスキップ
                logger.debug(f"[DEBUG] タイトルのみの小さなテーブルをスキップ: '{title_text}' at 行{start_row}-{end_row}")
                return
        
        # 非表形式のテキスト（対象分析装置など）をチェック
        if self._is_plain_text_region(sheet, region):
            logger.debug(f"[DEBUG] 非表形式テキストとして処理: 行{start_row}-{end_row}")
        ...
        try:
            ...
            if found_title_in_region:
                logger.debug(f"[DEBUG] タイトル行が領域先頭に含まれているためスキップ: '{title_text}' at 行{start_row}")
        ...
        if not header_row:
            ...
            if 1 < len(unique_cols) <= 8:
                ...
                try:
                    ...
                    if len(col_stats) >= 2:
                        ...
                        if left['distinct'] == 1 and right['distinct'] > 1 and left['nonempty'] / max(1, total_rows) < 0.95:
                            logger.debug(f"[DEBUG] unique_cols heuristic: dropping left repeated column {left['col']} in favor of {right['col']}")
                ...
                # 平均が0.5以上なら列ベース表と見なす
                if avg_nonempty >= 0.5:
                    logger.debug(f"CONV_UNIQUECOLS sheet={getattr(sheet,'title',None)} region={region} unique_cols={unique_cols} avg_nonempty={avg_nonempty:.2f}")
                    table_data = []
                    source_rows = []
            ...
            if table_data:
                logger.debug(f"[DEBUG] unique_cols-based table used: cols={unique_cols}, rows={len(table_data)}")
                ...
                try:
                    ...
                    logger.debug(f"[DEBUG-DUMP] unique_cols context: {ctx}")
                    for i, r in enumerate(table_data[:5]):
                        logger.debug(f"[DEBUG-DUMP] unique_cols table_data row {i}: {r}")
                except Exception as _e:
                    logger.debug(f"[DEBUG-DUMP] failed unique_cols dump: {_e}")
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug("領域タイトルの検出に失敗 (sheet=%s): %s", getattr(sheet, 'title', sheet), e)
                        title_in_region = None
                    if sheet_name == 'XMLファイル自動生成' or title_in_region == 'XMLファイル自動生成':
                        logger.debug('[DEBUG-TRACE] Detected target sheet/region for deep dump: XMLファイル自動生成')
                        logger.debug(f"[DEBUG-TRACE] region={region}")
                        logger.debug(f"[DEBUG-TRACE] unique_cols={unique_cols}")
                        # first_row_valsが存在する場合はダンプ
                        if 'first_row_vals' in locals():
                            logger.debug(f"[DEBUG-TRACE] first_row_vals={first_row_vals}")
                        if 'merge_into_left' in locals():
                            logger.debug(f"[DEBUG-TRACE] merge_into_left={merge_into_left}")
                        if 'merge_map' in locals():
                            logger.debug(f"[DEBUG-TRACE] merge_map={merge_map}")
                        # ヘッダー関連の構造体が存在する場合
                        for name in ('header_positions', 'final_groups', 'compressed_headers', 'group_positions'):
                            if name in locals():
                                logger.debug(f"[DEBUG-TRACE] {name}={locals()[name]}")
                            ...
                            for rr in range(region[0], min(region[0]+6, region[1]+1)):
                                ...
                                for c in range(region[2], region[3]+1):
                                    try:
                                        ...
                                        v = None
                                    rowvals.append((c, v))
                                logger.debug(f"[DEBUG-TRACE] raw row {rr}: {rowvals}")
                        except Exception as _e:
                            logger.debug(f"[DEBUG-TRACE] failed to dump raw rows: {_e}")
                except Exception as _e:
                    logger.debug(f"[DEBUG-TRACE] deep dump failed: {_e}")
                # 列ヘッダーが無ければプレーンな表として出力
                logger.debug(f"[DEBUG] 出力前テーブルプレビュー(unique_cols): rows={len(table_data)}, first_row={table_data[0] if table_data else None}")
                # デバッグ用にtable_dataの形状と最初の行をダンプ
                try:
                    cols = max(len(r) for r in table_data) if table_data else 0
                except (ValueError, TypeError):
                    cols = 0
                logger.debug(f"[DEBUG] _output_markdown_table called (unique_cols path): rows={len(table_data)}, max_cols={cols}")
                for i, r in enumerate(table_data[:10]):
                    logger.debug(f"[DEBUG] table_data row {i} cols={len(r)}: {r}")
                try:
                    # 以前の行と重複する可能性のある事前出力行を削除
                    logger.debug(f"[DEBUG][_prune_call_unique] sheet={sheet.title} before_prune rows={len(table_data) if table_data else 0} source_rows_sample={source_rows[:10] if source_rows else None}")
                    table_data, source_rows = self._prune_emitted_rows(sheet.title, table_data, source_rows)
                    logger.debug(f"[DEBUG][_prune_result_unique] sheet={sheet.title} after_prune rows={len(table_data) if table_data else 0} source_rows_sample={source_rows[:10] if source_rows else None}")
                except (ValueError, TypeError) as e:
                    logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                ...
                try:
                    ...
                    logger.debug(f"[DEBUG][_pre_output_call] path=unique_cols sheet={getattr(sheet, 'title', None)} rows={rows_len} source_rows_sample={src_sample}")
                except (ValueError, TypeError) as e:
                    logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
                try:
                    ...
                    try:
                        ...
                        logger.debug(f"DEFER_TABLE unique_cols sheet={sheet.title} anchor={anchor} rows={len(table_data)}")
                    except (ValueError, TypeError):
        ...
        if table_data:
            logger.debug(f"[DEBUG] 出力前テーブルプレビュー: rows={len(table_data)}, first_row={table_data[0] if table_data else None}")
            # 出力前にtable_dataの形状をダンプ
            try:
                cols = max(len(r) for r in table_data) if table_data else 0
            except (ValueError, TypeError):
                cols = 0
            logger.debug(f"[DEBUG] _output_markdown_table called (header/data path): rows={len(table_data)}, max_cols={cols}")
            for i, r in enumerate(table_data[:10]):
                logger.debug(f"[DEBUG] table_data row {i} cols={len(r)}: {r}")
            try:
                ...
                approx_rows = None
            try:
                logger.debug(f"[DEBUG][_prune_call_headerdata] sheet={sheet.title} before_prune rows={len(table_data) if table_data else 0} approx_rows_sample={approx_rows[:10] if approx_rows else None}")
                table_data, approx_rows = self._prune_emitted_rows(sheet.title, table_data, approx_rows)
                logger.debug(f"[DEBUG][_prune_result_headerdata] sheet={sheet.title} after_prune rows={len(table_data) if table_data else 0} approx_rows_sample={approx_rows[:10] if approx_rows else None}")
            except (ValueError, TypeError) as e:
                logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
            ...
            try:
                ...
                rows_len = len(table_data) if table_data else 0
                logger.debug(f"[DEBUG][_pre_output_call] path=header_data sheet={sheet.title} rows={rows_len} source_rows_sample={src_sample}")
            except (ValueError, TypeError) as e:
                logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
            ...
            try:
                if hasattr(self, '_last_group_positions') and self._last_group_positions:
                    ...
                    logger.debug(f"[DEBUG] _output_right_side_plain_text: actual_end_col={actual_end_col} (from group_positions={self._last_group_positions})")
                else:
                    actual_end_col = end_col
                    logger.debug(f"[DEBUG] _output_right_side_plain_text: actual_end_col={actual_end_col} (from region end_col)")
            except Exception as e:
                actual_end_col = end_col
                logger.debug(f"[DEBUG] _output_right_side_plain_text: actual_end_col={actual_end_col} (exception: {e})")
    ...
    def _output_right_side_plain_text(self, sheet, region: Tuple[int, int, int, int], actual_end_col: int = None):
        ...
        logger.debug(f"[DEBUG] _output_right_side_plain_text: rows={start_row}-{end_row}, cols={end_col+1}-{max_col}")
        for row_num in range(start_row, end_row + 1):
            right_texts = []
            for col_num in range(end_col + 1, max_col + 1):
                ...
                if cell.value is not None:
                    ...
                    if text:
                        ...
                        logger.debug(f"[DEBUG] _output_right_side_plain_text: 行{row_num}列{col_num} text='{text}'")
            # 右側にテキストがあれば出力
            if right_texts:
    ...
    def _is_plain_text_region(self, sheet, region: Tuple[int, int, int, int]) -> bool:
        ...
        try:
            ...
            logger.debug(f"[DEBUG][_is_plain_text_region_entry] sheet={getattr(sheet,'title',None)} region={start_row}-{end_row},{start_col}-{end_col} rows={rows} cols={cols}")
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        logger.debug(f"PLAIN_ENTRY sheet={getattr(sheet,'title',None)} region={start_row}-{end_row},{start_col}-{end_col} non_empty={non_empty_cells} total={total_cells}")
        ...
        try:
            ...
            if row_count == 1 and len(tokens) >= 2 and avg_len <= 60:
                logger.debug(f"[DEBUG] 単一行トークン複数 -> 表扱い: 行{start_row}〜{end_row}, tokens={len(tokens)}, avg_len={avg_len:.1f}")
                return False
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
            ...
            if len(content_cols) == 2:
                ...
                for r in range(start_row, end_row + 1):
                    try:
                        ...
                        logger.debug("左右列セル値の取得に失敗 (sheet=%s, row=%s): %s", getattr(sheet, 'title', sheet), r, e)
                        lv = None
                        rv = None
        ...
        fill_ratio = (sum(row_counts) / total_cells) if total_cells > 0 else 0
        ...
        if non_empty_cells > 0 and (path_like_count / non_empty_cells) > 0.25:
            ...
            if border_cols:
                logger.debug(f"[DEBUG] パス/XML多だが縦罫線で列境界が検出されたため表として扱います: {border_cols}")
                return False

            logger.debug(f"[DEBUG] プレーンテキスト判定(パス/XML多): 行{start_row}〜{end_row}, path_like={path_like_count}/{non_empty_cells}")
            return True

        # ルール2: 平均セル長が大きく、行数が少なめなら説明文ブロック
        if row_count <= 8 and avg_len > 60:
            logger.debug(f"[DEBUG] プレーンテキスト判定(長文多): 行{start_row}〜{end_row}, avg_len={avg_len:.1f}")
            return True
        ...
        try:
            if row_count == 1 and cols_with_content >= 2 and avg_len < 40:
                logger.debug(f"[DEBUG] 単一行短文複数列は表扱い: 行{start_row}〜{end_row}, cols_with_content={cols_with_content}, avg_len={avg_len:.1f}")
                return False
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        if row_count <= 2 and non_empty_cells <= 6 and long_count > 0:
            logger.debug(f"[DEBUG] 単純テキスト判定(少行で長文): 行{start_row}〜{end_row}, long_count={long_count}")
            return True
        ...
        if avg_row_nonempty >= 2 and row_std <= max(1.5, avg_row_nonempty * 0.6) and cols_with_content >= 2:
            ...
            logger.debug(f"[DEBUG] 表構造検出: 行{start_row}〜{end_row}, avg_row_nonempty={avg_row_nonempty:.1f}, row_std={row_std:.2f}, cols_with_content={cols_with_content}")
            return False
    ...
    def _build_table_with_header_row(self, sheet, region: Tuple[int, int, int, int], 
        ...
        logger.debug(f"[DEBUG] ヘッダー行{header_row}でテーブルを構築中...")
        ...
        if header_min_col < start_col or header_max_col > end_col or actual_start_row < start_row:
            logger.debug(f"[DEBUG] ヘッダー行により範囲を拡張: 行{start_row}-{end_row} → {actual_start_row}-{actual_end_row}, 列{start_col}-{end_col} → {header_min_col}-{header_max_col}")
        ...
        for col in range(start_col, end_col + 1):
            ...
            try:
                ...
                if is_master_col or header_height > 1:
                    ...
                    for p in dedup_parts:
                        ...
                        if drop:
                            logger.debug(f"[DEBUG] ヘッダからデータトークン除外: '{p}' at 列{col} (occurrence_fraction={frac:.2f}, len={plen})")
                            continue
                        filtered_parts.append(p)
            ...
            if combined:
                ...
                if col_ratio < 0.2 and not keep_despite_low_ratio:
                    # 注記っぽい列としてスキップ
                    logger.debug(f"[DEBUG] ヘッダー候補除外(注記っぽい列): '{combined}' at 列{col} (col_nonempty={col_ratio:.2f})")
                    continue
                ...
                try:
                    ...
                    logger.debug(f"[DEBUG] ヘッダー書式適用エラー（無視）: {e}")

                headers.append(combined)
                header_positions.append(col)
                if is_master_col:
                    logger.debug(f"[DEBUG] 結合ヘッダー検出・展開(マスター含む): '{combined}' at 列{col}")
                else:
                    logger.debug(f"[DEBUG] ヘッダー検出(結合): '{combined}' at 列{col}")
        
        logger.debug(f"[DEBUG] 最終ヘッダー: {headers}")
        logger.debug(f"[DEBUG] ヘッダー位置: {header_positions}")
        ...
        try:
            ...
            if total_headers > 0 and (nonempty_headers / total_headers) < 0.20 and header_row + 1 <= end_row:
                logger.debug(f"[DEBUG] ヘッダーがほとんど空のため、header_rowを1行下にシフトして再試行します (from {header_row} -> {header_row+1})")
                ...
                if any(shifted_headers):
                    ...
                    logger.debug(f"[DEBUG] シフト後ヘッダー採用: headers={headers}, positions={header_positions}, header_row={header_row}")
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        try:
            ...
            if len(filtered) != len(headers):
                ...
                logger.debug(f"[DEBUG] 空ヘッダー列を削除: headers={headers}, positions={header_positions}")
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        if headers and (data_like_count / len(headers)) > 0.4 and not has_merged_header:
            logger.debug(f"[DEBUG] ヘッダーっぽい行がデータ寄りと判断({data_like_count}/{len(headers)})、ヘッダー処理をスキップします")
            return self._trim_edge_empty_columns(self._build_table_data_with_merges(sheet, region, merged_info))
        if has_merged_header:
            logger.debug(f"[DEBUG] 結合セルを含むヘッダーが検出されたため、データ寄り判定を無視してヘッダー処理を継続します")

        if len(headers) < 2:
            logger.debug(f"[DEBUG] ヘッダーが不十分、従来方式を使用")
            return self._trim_edge_empty_columns(self._build_table_data_with_merges(sheet, region, merged_info))
    ...
    # --- ヘッダーの連続重複を圧縮する ---
        ...
        logger.debug(f"[DEBUG] ヘッダーグループ (元): {groups}")
        logger.debug(f"[DEBUG] ヘッダーグループ (最終): {final_groups}")
        logger.debug(f"[DEBUG] 圧縮後ヘッダー: {compressed_headers}")
        # 詳細ダンプ: デバッグ用にヘッダー周りの内部状態を出力
        try:
            logger.debug(f"[DEBUG-DUMP] headers={headers}")
            logger.debug(f"[DEBUG-DUMP] header_positions={header_positions}")
            logger.debug(f"[DEBUG-DUMP] group_positions={group_positions}")
            logger.debug(f"[DEBUG-DUMP] final_groups={final_groups}")
            logger.debug(f"[DEBUG-DUMP] compressed_headers={compressed_headers}")
            logger.debug(f"[DEBUG-DUMP] merged_info_keys_sample={list(merged_info.keys())[:20]}")
            ...
            for rr in range(header_row + 1, min(header_row + 6, end_row + 1)):
                ...
                logger.debug(f"[DEBUG-DUMP] raw row {rr}: {rowvals}")
        except Exception as _e:
            logger.debug(f"[DEBUG-DUMP] failed to dump internal state: {_e}")
        ...
        logger.debug(f"[DEBUG] group_column_ranges={group_column_ranges}")
        ...
        logger.debug(f"[DEBUG] group_column_priority={group_column_priority}")
        ...
        for row_num in range(data_start_row, end_row + 1):
            ...
            for g_idx, cols_priority in enumerate(group_column_priority):
                ...
                if row_num == 28 and g_idx == 1:  # 行28グループ1(初期値)を特別追跡
                    logger.debug(f"[DEBUG] 行28グループ1候補: {cols_priority[:5]}")
                for col_candidate in cols_priority:
                    ...
                    if row_num == 28 and g_idx == 1 and col_candidate <= 15:  # 最初の数列のみ
                        logger.debug(f"[DEBUG] 行28列{col_candidate}: content='{content}', bool={bool(content and content.strip())}")
                ...
                if row_num == 28 or row_num <= header_row + 3:  # 行28を特別に追跡
                    logger.debug(f"[DEBUG] 行{row_num}列{chosen_col}({header_name}): -> '{chosen_content}'")
        ...
        logger.debug(f"[DEBUG] table_data構築完了: {len(table_data)}行")
        if table_data:
            logger.debug(f"[DEBUG] table_data[0] (ヘッダー): {table_data[0]}")
            if len(table_data) > 1:
                logger.debug(f"[DEBUG] table_data[1] (最初のデータ行): {table_data[1]}")
            if len(table_data) > 2:
                logger.debug(f"[DEBUG] table_data[2] (2番目のデータ行): {table_data[2]}")
        
        # 2列最適化は無効化（3列テーブルは3列のまま出力する）
        # ユーザー要望: 「3列のものは3列で表示すべき」
        # 将来の拡張のため関数本体は残しておく
        logger.debug(f"[DEBUG] 2列最適化は無効化されています（3列テーブルは3列のまま出力）")
    ...
    def _find_table_title_in_region(self, sheet, region: Tuple[int, int, int, int], 
        ...
        for row in range(search_start, search_end):
            ...
            if _is_row_in_other_table(row):
                logger.debug(f"[DEBUG] タイトル候補除外(他テーブル領域内): 行{row}")
                continue
                ...
                if cell.value:
                    ...
                    if len(text) <= 80 and len(text.split()) <= 8:
                        ...
                        if start_row <= row <= end_row:
                            ...
                            col_ratio = self._column_nonempty_fraction(sheet, start_row, end_row, col)
                            if col_ratio < 0.2:
                                logger.debug(f"[DEBUG] タイトル候補除外(注記っぽい列): '{text}' at 行{row}列{col} (col_nonempty={col_ratio:.2f})")
                                continue

                        distance = abs(row - start_row)
                        row_relation = 0 if row < start_row else (1 if row == start_row else 2)
                        title_candidates.append((text, distance, row, col, 'general', row_relation))
                        logger.debug("[DEBUG] タイトル候補: '{}' at 行{}列{}, 距離{}".format(text, row, col, distance))
        ...
        if title_candidates:
            ...
            if best_row_relation == 1 and not is_single_row_table:
                logger.debug(f"[DEBUG] タイトルがテーブル開始行と同じ行のため、タイトルなしとして扱う: '{best_title[0]}' at 行{best_row}")
                self._last_table_title_row = None
                return None
            ...
            if len(same_row_candidates) > 1:
                ...
                logger.debug("[DEBUG] タイトル選択（結合）: '{}' (type={}, row={}, skip_formatting={})".format(combined_title, best_title[4], best_title[2], skip_formatting))
            ...
            logger.debug("[DEBUG] タイトル選択: '{}' (type={}, row={}, skip_formatting={})".format(title_text, best_title[4], best_title[2], skip_formatting))
            return title_text

        # タイトルが見つからない場合は以前のタイトル行をクリア
        self._last_table_title_row = None
        logger.debug("[DEBUG] テーブルタイトルが見つかりませんでした")
        return None
    ...
    def _find_table_header_row(self, sheet, region: Tuple[int, int, int, int]) -> Optional[Tuple[int, int]]:
        ...
        logger.debug(f"[DEBUG] ヘッダー検索: 行{start_row}〜{min(start_row + 5, end_row + 1)}")
        logger.debug(f"[DEBUG][_find_table_header_row_entry] sheet={sheet.title} region={start_row}-{end_row},{start_col}-{end_col}")
            ...
            for height in (1, 2, 3):
                ...
                if height > 1 and multirow_frac < 0.25:
                    ...
                    if nonempty >= total_columns * 0.5:
                        logger.debug(f"[DEBUG] 複数行ヘッダ候補を維持（非空セルが多い）: row={row}, height={height}, multirow_frac={multirow_frac:.2f}, nonempty={nonempty}/{total_columns}")
                    else:
                        logger.debug(f"[DEBUG] 複数行ヘッダ候補をスキップ（実際には単一セル内改行が多い）: row={row}, height={height}, multirow_frac={multirow_frac:.2f}, nonempty={nonempty}/{total_columns}")
                        continue
                ...
                # デバッグ
                logger.debug(f"[DEBUG] 行{row}..{row+height-1} combined header_values (first16): {header_values[:16]}")
                logger.debug(f"[DEBUG] 行{row} height={height} group_count={group_count}, nonempty_cols={nonempty}")
                ...
                for c in range(start_col, start_col + total_columns):
                    try:
                        ...
                        logger.debug("下端罫線の確認に失敗 (row=%s, col=%s): %s", bottom_row, c, e)
                        continue
                ...
                def _row_header_likeness(rnum):
                    try:
                        ...
                        logger.debug("行ヘッダーらしさスコアの計算に失敗 (row=%s): %s", rnum, e)
                        return 0.0

                likeness_score = _row_header_likeness(bottom_row)
                # デバッグ print for inspection
                logger.debug(f"[DEBUG] header-likeness(bottom_row={bottom_row})={likeness_score:.3f}")
                logger.debug(f"[DEBUG] header_border_fraction(bottom_row={bottom_row})={header_border_fraction:.3f}")
                ...
                logger.debug(f"[DEBUG] extended_group_count(row={row})={extended_group_count} (original group_count={group_count})")
                ...
                if first_row_bonus > 0:
                    logger.debug(f"[DEBUG] 最初の行ボーナス適用: row={row}, border_fraction={header_border_fraction:.3f} -> {adjusted_border_fraction:.3f}")
        ...
        if best_row:
            ...
            try:
                if best_height and best_height > 1:
                    ...
                    if (best_group_count - group_count_one) <= 1:
                        logger.debug(f"[DEBUG] ヘッダー高さの見直し: 複数行によるグループ増分が小さいため単一行を優先します (row={best_row}, before_height={best_height}, groups_before={best_group_count}, groups_one={group_count_one})")
                        best_height = 1
                        self._detected_header_height = best_height
            except (ValueError, TypeError) as e:
                logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
            ...
            try:
                if best_height and best_height > 1:
                    ...
                    if group_count_bottom >= best_group_count and nonempty_bottom >= max(2, int(total_columns * 0.6)):
                        logger.debug(f"[DEBUG] ヘッダー行選択の調整: 下端行が十分に代表的なヘッダーのため下端行を単一行ヘッダーにします (from row={best_row}, height={best_height} -> row={bottom_row}, height=1)")
                        best_row = bottom_row
                        best_height = 1
                        try:
                            self._detected_header_start = best_row
            ...
            logger.debug(f"[DEBUG] ヘッダー行選択 (罫線優先): 行{best_row} (groups={best_group_count}, height={best_height})")
            return (best_row, best_height)

        # ヘッダー行が見つからなかった場合
        logger.debug(f"[DEBUG] ヘッダー行が見つかりませんでした")
        return None
    ...
    def _get_merged_cell_info(self, sheet, region: Tuple[int, int, int, int]) -> Dict[str, Any]:
        ...
        try:
            logger.debug(f"[DEBUG] 結合セル情報取得開始: region={region}")
            for merged_range in sheet.merged_cells.ranges:
                logger.debug(f"[DEBUG] 結合セル範囲発見: 行{merged_range.min_row}〜{merged_range.max_row}, 列{merged_range.min_col}〜{merged_range.max_col}")
                
                # 結合セルがテーブル領域と重なっているかチェック（部分的な重なりも含む）
                if (merged_range.max_row >= start_row and merged_range.min_row <= end_row and
                    merged_range.max_col >= start_col and merged_range.min_col <= end_col):
                    
                    logger.debug(f"[DEBUG] 結合セルが領域に重なる: 行{merged_range.min_row}〜{merged_range.max_row}, 列{merged_range.min_col}〜{merged_range.max_col}")
                    ...
                    for row in range(actual_start_row, actual_end_row + 1):
                        for col in range(actual_start_col, actual_end_col + 1):
                            ...
                            logger.debug(f"[DEBUG] 結合セル登録: {key} -> master({merged_range.min_row}, {merged_range.min_col})")
                else:
                    logger.debug(f"[DEBUG] 結合セルが領域外: 行{merged_range.min_row}〜{merged_range.max_row}, 列{merged_range.min_col}〜{merged_range.max_col}")
        except Exception as e:
            logger.debug(f"[DEBUG] 結合セル情報取得エラー: {e}")
    ...
    def _consolidate_merged_rows(self, table_data: List[List[str]], merged_info: Dict[str, Any],
        ...
        logger.debug(f"[DEBUG] _consolidate_merged_rows called: table_data rows={len(table_data)}, start_row={start_row}, start_col={start_col}, end_col={end_col}")
        logger.debug(f"[DEBUG] merged_info keys sample: {list(merged_info.keys())[:10]}")
        ...
        for row_idx in range(len(table_data)):
            ...
            logger.debug(f"[DEBUG] Processing row_idx={row_idx}, actual_row_num={actual_row_num}, current_row={current_row}")
            ...
            if multi_row_merges:
                ...
                logger.debug(f"[DEBUG] Row {actual_row_num} has {len(multi_row_merges)} multi-row merges, max_span={max_span}")
                ...
                for next_row_offset in range(1, max_span):
                    ...
                    for next_col_idx in range(len(next_row)):
                        ...
                        if next_cell and str(next_cell).strip():
                            ...
                            logger.debug(f"[DEBUG] Row {next_actual_row_num} has non-merged data at col {next_col_idx}: {next_cell}")
                            break
                    
                    if has_non_merged_data:
                        logger.debug(f"[DEBUG] Row {next_actual_row_num} has non-merged data, not consolidating")
                    else:
                        logger.debug(f"[DEBUG] Row {next_actual_row_num} is empty (merged cell only), consolidating")
                        for next_col_idx in range(len(next_row)):
        ...
        logger.debug(f"[DEBUG] _consolidate_merged_rows: {len(table_data)} rows -> {len(rows_to_keep)} rows (skipped {len(rows_to_skip)} rows)")
        return rows_to_keep
    ...
    def _build_table_data_with_merges(self, sheet, region: Tuple[int, int, int, int], 
        ...
        """
        start_row, end_row, start_col, end_col = region
        logger.debug(f"[DEBUG] _build_table_data_with_merges実行: region={region}")
        
        # ヘッダー行を検出
        header_info = self._find_table_header_row(sheet, region)
        if header_info:
            header_row, header_height = header_info
            logger.debug(f"[DEBUG] ヘッダー行発見: {header_row}, height={header_height}, テーブルをヘッダー行から開始")
            ...
            if header_min_col < start_col or header_max_col > end_col:
                logger.debug(f"[DEBUG] ヘッダー行により列範囲を拡張: {start_col}-{end_col} → {header_min_col}-{header_max_col}")
                start_col = header_min_col
                end_col = header_max_col
        else:
            header_row = None
            header_height = 1
            logger.debug(f"[DEBUG] ヘッダー行なし、最初の行から開始")
            actual_start_row = start_row
                ...
                if key in merged_info and merged_info[key]['is_merged']:
                    ...
                    if (row_num == merge_info['master_row'] and 
                        ...
                        if merge_info['span_rows'] > 1 or merge_info['span_cols'] > 1:
                            logger.debug(f"[DEBUG] 結合セル検出: '{content}' を範囲 (行:{merge_info['span_rows']}, 列:{merge_info['span_cols']}) に展開")
                else:
        ...
        logger.debug(f"[DEBUG-DUMP] raw_table_data rows={len(raw_table_data)} sample (first 6):")
        for i, r in enumerate(raw_table_data[:6]):
            logger.debug(f"[DEBUG-DUMP] raw row {i+actual_start_row}: cols={len(r)} -> {r}")
        
        # 空行も含めてすべての行を保持(罫線で囲まれた空行もテーブルの一部)
        filtered_table_data = raw_table_data

        # dump filtered_table_data sample for diagnostics
        logger.debug(f"[DEBUG-DUMP] filtered_table_data rows={len(filtered_table_data)} sample (first 6):")
        for i, r in enumerate(filtered_table_data[:6]):
            logger.debug(f"[DEBUG-DUMP] filtered row {i+actual_start_row}: cols={len(r)} -> {r}")
        ...
        try:
            ...
            # ロギングのみ行い、処理を継続
            logger.debug('[TRACE-USE-HEADER]', str(region), f'useful_columns_before={useful_columns}')
        ...
        try:
            ...
            useful_columns = sorted(set(useful_columns))
            logger.debug(f"[TRACE-USEFUL-DECISION] region={region} initial_counts={col_counts} kept_by_guard={kept_by_guard} final_useful={useful_columns}")
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        try:
            ...
            # 診断を出力
            logger.debug('[COLUMN-MAP] region_abs_cols={} ->'.format((start_col, end_col)))
            for t in per_column_diag:
                abs_col, rel, in_initial, header_present, header_texts, data_count, reason = t
                logger.debug(f"[COLUMN-MAP] col={abs_col} rel={rel} initial={in_initial} header_present={header_present} header_texts={header_texts} data_count={data_count} -> {reason}")
        except (ValueError, TypeError) as e:
            logger.debug(f"[DEBUG] 型変換エラー（無視）: {e}")
        ...
        # 列圧縮ステップ後にダンプ
        logger.debug(f"[DEBUG-DUMP] after useful_columns compression: useful_columns={useful_columns}, table_rows={len(table_data)} sample (first 6):")
        for i, r in enumerate(table_data[:6]):
            logger.debug(f"[DEBUG-DUMP] compressed row {i}: cols={len(r)} -> {r}")
        ...
        if table_data:
            ...
            logger.debug(f"[DEBUG-DUMP] per-column nonempty counts (after useful_columns): {col_nonempty_counts}")
            ...
            for (a, b) in groups:
                ...
                if nonempty_cols_in_group > 1:
                    ...
                    logger.debug(f"[HEADER-GROUP-SKIP-MULTIDATA] expanded group {(a,b)} into singletons because nonempty_cols={nonempty_cols_in_group}")
                else:
                    final_groups.append((a, b))
        ...
        if len(table_data) > 1 and len(table_data[0]) == 3:
            ...
            headers = table_data[0]
            logger.debug(f"[DEBUG] _build_table_data_with_merges内で2列最適化チェック: headers={headers}")
            ...
            if self._is_setting_item_pattern_tabledata(table_data, 1, 2):
                ...
                logger.debug(f"[DEBUG] 2列最適化実行: {headers[0]} | {headers[2]}")
                ...
                # 2列最適化決定のための追加診断ダンプ
                logger.debug(f"[DEBUG-DUMP] 2col optimization: total_data_rows={total_data_rows}, matched={matched}, required={required}")
                # 決定に使用された行のサンプルを表示
                for j, r in enumerate(table_data[1: min(len(table_data), 1+10) ]):
                    logger.debug(f"[DEBUG-DUMP] data row sample {j+1}: {r}")

                if matched >= required:
                    logger.debug(f"[DEBUG] 2列最適化成功、{len(optimized_table)}行のテーブルを返す (matched={matched}/{total_data_rows})")
                    return optimized_table
                else:
                    logger.debug(f"[DEBUG] 2列最適化スキップ（マッチ行不足: {matched}/{total_data_rows}、必要={required}）")
            else:
                logger.debug(f"[DEBUG] パターンマッチせず（_build_table_data_with_merges内）")
    ...
    def _collapse_repeated_sequence(self, parts: List[str]) -> List[str]:
        ...
        try:
            ...
            logger.debug("繰り返しパターンの検出に失敗: %s", e)
            return parts
    ...
    def _escape_cell_for_table(self, text: str) -> str:
        ...
        try:
            ...
            logger.debug("セルテキストのエスケープに失敗: %s", e)
            return str(text)
    ...
    def _output_markdown_table(self, table_data: List[List[str]], source_rows: Optional[List[int]] = None, sheet_title: Optional[str] = None):
        ...
        if source_rows and len(source_rows) >= 2 and source_rows[0] <= 4:
            ...
            logger.debug(f"[DEBUG][{sheet_title}] テーブル生成: rows={source_rows[:5]}, cols={len(table_data[0]) if table_data else 0}, caller={' <- '.join(caller_info)}")
    ...
    # ヘッダーを出力
        ...
        if source_rows and sheet_title:
            try:
                ...
                logger.debug("セル→Markdownマッピングの取得に失敗 (sheet=%s): %s", sheet_title, e)
                sheet_map = None
