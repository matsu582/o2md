...
class ExcelToMarkdownConverter:
    ...
    def _convert_pdf_page_to_png(self, pdf_path: str, page_index: int, dpi: int,
        ...
        try:
            ...
            debug_print(f"[DEBUG] ImageMagick command: {' '.join(cmd)}")
    ...
    def _render_sheet_isolated_group(self, sheet, shape_indices: List[int], dpi: int = 600, cell_range: Optional[Tuple[int,int,int,int]] = None) -> Optional[str]:
        ...
        **PRODUCTION METHOD - RECOMMENDED FOR ALL USE CASES**
        ...
        try:
            ...
            debug_print(f"[DEBUG][_iso_entry] sheet={sheet.title} drawing_path={drawing_path} shape_indices={shape_indices}")
            if drawing_path not in z.namelist():
                drawing_path = drawing_path.replace('worksheets', 'drawings')
                if drawing_path not in z.namelist():
                    debug_print(f"[DEBUG][_iso_entry] sheet={sheet.title} drawing_path not found in archive after normalization: {drawing_path}")
                    return None
            ...
            for node in drawing_xml:
                ...
                if lname in ('twocellanchor', 'onecellanchor') and self._anchor_has_drawable(node):
                    anchors.append(node)
            if not anchors:
                debug_print(f"[DEBUG][_iso_entry] sheet={sheet.title} no drawable anchors found in drawing xml")
                return None
                        ...
                        try:
                            ...
                            if max_data_col > 0:
                                ...
                                if e_col > max_allowed_col:
                                    debug_print(f"[DEBUG][_iso_entry] Limiting e_col from {e_col} to {max_allowed_col} (max_data_col={max_data_col})")
                                    e_col = max_allowed_col
                        except Exception as limit_err:
                            debug_print(f"[DEBUG][_iso_entry] Failed to limit cell_range: {limit_err}")
            ...
            try:
                ...
                try:
                    debug_print(f"[DEBUG][_iso_entry] sheet={sheet.title} tmp_xlsx will be created at: {tmp_xlsx}")
                except Exception:
                    pass
                            ...
                            try:
                                ...
                                if hfs:
                                    ...
                                    try:
                                        ...
                                        pass
                                    try:
                                        debug_print(f"[DEBUG][_iso_hdrfoot_sweep] removed {removed_count} headerFooter from {relp} and forced fit-to-page/margins")
                                    except Exception:
                                        pass
                            except Exception:
                                pass
                ...
                try:
                    ...
                    debug_print(f"[DEBUG] saved group workbook: {dbg_copy}")
                    try:
                        st = os.stat(dbg_copy)
                    ...
                    try:
                        ...
                        try:
                            ...
                            debug_print(f"[DEBUG] created fixed workbook (inline): {fixed_candidate}")
                            try:
                                ...
                                try:
                                    ...
                                    debug_print(f"[DEBUG] openpyxl resaved fixed workbook: {fixed_candidate}")
                        ...
                        try:
                            ...
                            try:
                                ...
                                debug_print(f"[DEBUG] wrote fixed workbook ids: {ids_fn} (count={len(ids)})")
                            except Exception:
                                pass
                        except Exception:
                            pass
                        ...
                        if os.path.exists(repaired_candidate):
                            ...
                            debug_print(f"[DEBUG] using repaired workbook for conversion: {src_for_conv}")
                        elif os.path.exists(fixed_candidate):
                            src_for_conv = fixed_candidate
                            debug_print(f"[DEBUG] using fixed workbook for conversion: {src_for_conv}")
                        else:
                        ...
                        try:
                            ...
                            try:
                                debug_print(f"[DEBUG][_iso_conv_choice] sheet={sheet.title} src_for_conv={src_for_conv}")
                            except Exception:
                                pass
                            ...
                            if proc_conv.returncode == 0 and os.path.exists(compat_path):
                                debug_print(f"[DEBUG] saved LibreOffice-resaved compatible workbook: {compat_path}")
                            else:
                                ...
                                if candidates:
                                    debug_print(f"[DEBUG] detected LibreOffice output candidate: {candidates[0]}")
                        except Exception as _e:
                            try:
                ...
                try:
                    ...
                    if try_extract_image_direct and img_src_path:
                        ...
                        try:
                            debug_print(f"[DEBUG][_iso_direct] sheet={sheet.title} direct-extract candidate={img_src_path} will produce {out_img_path}")
                        except Exception:
                            pass
                        ...
                        try:
                            ...
                            try:
                                debug_print(f"[DEBUG] extracted embedded image directly: {out_img_path}")
                            except Exception:
                                pass
                            # cleanup tmp dirs
                            try:
                            ...
                            try:
                                debug_print(f"[DEBUG][_iso_direct_return] sheet={sheet.title} returned_direct_image={os.path.basename(out_img_path)} src={img_src_path}")
                            except Exception:
                                pass
                        ...
                        for sp in sheet_paths:
                            if os.path.exists(sp):
                                try:
                                    ...
                                    debug_print(f"[DEBUG] Applied fitToPage settings to {sp}")
                                except Exception as e:
                                    debug_print(f"[DEBUG] Failed to apply fitToPage settings: {e}")
                                except Exception:
                                    pass
                    except Exception:
                        pass
                ...
                try:
                    self._set_excel_fit_to_one_page(tmp_xlsx)
                    debug_print(f"[DEBUG] isolated group用にtmp_xlsxへpageSetup設定を適用: {tmp_xlsx}")
                except Exception as e:
                    print(f"[WARNING] isolated group tmp_xlsx pageSetup設定失敗: {e}")

                # Use Phase 1 foundation method for Excel→PDF conversion
                try:
                    debug_print(f"[DEBUG][_iso_conv_invoke] sheet={sheet.title} invoking LibreOffice to convert tmp_xlsx={tmp_xlsx} to PDF in {tmp_pdf_dir}")
                except Exception:
                    pass
                ...
                if pdf_path is None:
                    try:
                        ...
                        debug_print(f"[DEBUG][_iso_conv_fallback] sheet={sheet.title} falling back to original excel_file={getattr(self,'excel_file',None)} for conversion")
                    except Exception:
                        pass
                    try:
                ...
                try:
                    debug_print(f"[DEBUG][_iso_imagemagick] running image magick to produce {out_path}")
                except Exception:
                    pass
                ...
                if proc2.returncode != 0:
                    ...
                    try:
                        ...
                        pass
                    try:
                        debug_print(f"[DEBUG][_iso_im_convert_fallback] sheet={sheet.title} will fallback to using original excel_file={getattr(self,'excel_file',None)}")
                    except Exception:
                        pass
                    return None
                        ...
                        if cell_range:
                            ...
                            if os.path.exists(out_path):
                                ...
                                try:
                                    ...
                                    try:
                                        debug_print(f"[DEBUG] expected_w={expected_w} expected_h={expected_h} w_im={w_im} h_im={h_im} scale_x={scale_x} scale_y={scale_y}")
                                        debug_print(f"[DEBUG] page_box={page_box}")
                                    except Exception:
                                        pass
                                ...
                                if content_bbox:
                                    ...
                                    try:
                                        debug_print(f"[DEBUG] content_bbox={content_bbox} sheet_px_box=({sheet_l_px},{sheet_t_px},{sheet_r_px},{sheet_b_px})" )
                                    except Exception:
                                        try:
                                            debug_print(f"[DEBUG] fallback sheet cell coords: sl={col_x[s_col-1]}, st={row_y[s_row-1]}, er={col_x[e_col] if e_col < len(col_x) else 'NA'}, eb={row_y[e_row] if e_row < len(row_y) else 'NA'}")
                                        except Exception:
                                            pass
                                    try:
                                        debug_print(f"[DEBUG] crop_px=({lpx},{tpx},{rpx},{bpx})")
                                    except Exception:
                                        pass
                                else:
