...
- **Excel変換** (`x2md`): 表、グラフ、図形を含むワークシートを変換
...
- **MS Project変換** (`mpp2md`): MS Projectファイル(.mpp/.mpt/.mpx)をタスク一覧・スケジュール・進捗・担当者のMarkdownテーブルに変換。mpxjをGraalVM Native Imageでネイティブ化（実行時JRE不要）
...
- **検索エンジン用フィルタ** (`o2md-filter`): 文書をプレーンテキストに変換（stdin対応・ファイルタイプ自動判別）
- **Fess連携**: 全文検索サーバー[Fess](https://github.com/codelibs/fess)にo2md-filterを組み込み、MCPサーバーとしてAIアシスタントから検索可能（詳細は [fess/README.md](fess/README.md)）
...
### Tesseract OCR のインストール（オプショナル）

PDFや画像のOCRテキスト抽出にデフォルトで使用されます。
インストールしない場合、OCR機能は利用できません（manga-ocrまたはsarashina2.2-ocrで代替可能）。
...
| オプション | 説明 |
| --- | --- |
| `file` | 変換対象ファイル（省略時はstdinから読み込み） |
| `--ocr-engine` | OCRエンジンを指定（`tesseract`/`manga-ocr`/`sarashina`、デフォルト: `tesseract`） |
...
> **補足**: ローカルクローンから実行する場合は、コマンドの前に `uv run` を付けてください（例: `uv run o2md data.xlsx`）。
...
| ファイル種類 | 拡張子          | コマンド   | 主な機能                     |
| ------------ | --------------- | ---------- | ---------------------------- |
| Excel        | `.xlsx`, `.xls` | `x2md`     | 表、グラフ、図形、数式       |
...
- mpxjライブラリをGraalVM Native Imageでネイティブバイナリにコンパイル
- 実行時JRE不要
...
- [docling](https://github.com/docling-project/docling)表検出を使用するには追加のインストールが必要です
  - `pip install o2md[docling]` または `uv sync --extra docling`でインストール
  - 処理時間: 1ページあたり約7-15秒（CPUのみの場合）
- [manga-ocr](https://github.com/kha-white/manga-ocr)を使用するには追加のインストールが必要です
  - `pip install o2md[manga-ocr]` または `uv sync --extra manga-ocr`でインストール
- [sarashina2.2-ocr](https://huggingface.co/sbintuitions/sarashina2.2-ocr)を使用するには追加のインストールが必要です
  - `pip install o2md[sarashina]` または `uv sync --extra sarashina`でインストール
...
### MS Project変換 (`mpp2md`)
- mpp-readerネイティブバイナリが必要（o2md/bin/に同梱またはPATH上に配置）
- ネイティブバイナリはプラットフォーム固有（Linux x86_64）、他プラットフォームはソースからビルドが必要
- ビルドにGraalVM + native-imageが必要: `cd native/mpp-reader && ./gradlew nativeCompile`

### 画像OCR変換 (`img2md`)
- OCRの精度は画像の品質・解像度に依存します
- 手書き文字の認識精度は低い場合があります
- Tesseract OCRを使用するには事前にインストールが必要です
- manga-ocrを使用するには`pip install o2md[manga-ocr]`または`uv sync --extra manga-ocr`が必要です
- sarashina2.2-ocrを使用するには`pip install o2md[sarashina]`または`uv sync --extra sarashina`が必要です（GPU推奨）
...
│   ├── bin/                # ネイティブバイナリ
│   │   └── mpp-reader      # MS Projectリーダー（GraalVM Native Image）
│   ├── utils.py            # 共通ユーティリティ
│   └── omml_converter/     # OMML→LaTeX変換
├── native/                 # ネイティブバイナリビルドソース
│   └── mpp-reader/         # MS Projectリーダー（Java + Gradle + GraalVM）
├── tests/                  # テストスイート
└── input_files/            # テスト用入力ファイル
```
