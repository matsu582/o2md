...
使用例:
    ...
    # フォルダ内の全ファイルを一括変換（サブフォルダ含む）
    python o2md.py input_files/
    python o2md.py input_files/ -o output_all
...
def collect_target_files(folder_path: str) -> list:
    """フォルダ内の変換対象ファイルを再帰的に収集
...
def convert_folder(folder_path: str, output_dir: str = None, **kwargs) -> dict:
    """フォルダ内の全対象ファイルを一括変換

    サブフォルダも含めて再帰的に処理し、入力フォルダのディレクトリ構造を
    出力先に再現する。
    ...
    target_files = collect_target_files(str(folder))
...
使用例:
  ...
  python o2md.py ./input_folder/          # フォルダ一括変換
  python o2md.py ./input_folder/ -o out   # 出力先指定
        """
    )
