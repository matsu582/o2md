...
使用例:
    ...
    cat report.pdf | o2md-filter --type pdf
...
def resolve_file_type(file_path: str, type_hint: str = None) -> str:
    """ファイルのタイプを確定する
...
使用例:
  ...
  cat file.docx | o2md-filter        # stdinから入力
  cat file.pdf | o2md-filter --type pdf  # タイプ明示指定
...
対応形式:
  ...
  PDF (.pdf), 一太郎 (.jtd, .jtt), 画像 (.jpg, .png, etc.)
    ...
    parser.add_argument('--type', dest='file_type', default=None,
                        choices=['excel', 'word', 'powerpoint', 'pdf',
                                 'ichitaro', 'image'],
                        help='ファイルタイプを明示指定（stdin使用時に自動判別できない場合）')
    ...
    try:
        if args.file:
            ...
            file_type = resolve_file_type(args.file, args.file_type)
            ...
            # 一時ファイルに保存して処理
            suffix = _get_suffix_for_type(base_type, args.file_type)
            ...
            try:
                file_type = resolve_file_type(tmp_path, args.file_type)
                if file_type == 'unknown':
...
def _get_suffix_for_type(base_type: str, type_hint: str = None) -> str:
    ...
    if type_hint:
        ...
        return suffix_map.get(type_hint, '.bin')
