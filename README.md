...
Excel、Word、PowerPoint、PDF、一太郎ファイルを自動判定して**それっぽい**Markdownに変換するツール。
...
o2mdは、Microsoft Office文書（Excel、Word、PowerPoint）、PDF、および一太郎文書（.jtd/.jtt）を**それっぽい**Markdown形式に変換するPythonツールです。ファイルの種類を自動判定し、適切な変換エンジンを使用して処理します。
...
# テキスト抽出モード（画像リンク除去、.mdと.txt両方を出力）
...
| `--text`             | テキスト抽出モード（画像リンク除去、.mdと.txt両方を出力）  |
